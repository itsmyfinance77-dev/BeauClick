...
## V3.3 Story #149 split into `#149a` and `#149b`, 2026-09-12

`V33-DEC-037` ratified the split the read-only readiness audit of #149 recommended. **Frontend
decomposition and persona contract only** — no commercial value, no Legal wording, no schema, no
route, no new ADR and no implementation. The audit, against `master` `5fbf92d`, found that the
unsplit story bundled two independently deliverable outcomes with different personas and
security boundaries: the business owner's grant/revoke control on the staff roster #123 corrected,
and the reader's surface — which today does not exist for the persona #111 was built for, because
the only finance screen sits behind `ProGuard` inside the professional shell, ignores the additive
`accessMode`, selects `items[0]` when several workspaces exist and passes no settlements cursor.

| Item | Before | After | Outcome it owns |
|---|---|---:|---|
| #149 (`#149a`) | `status:proposed`, `priority:p2`, 5, "Grant management and a read-only finance persona for finance_read" | **`status:proposed`, `priority:p1`, 3**, retitled "Owner UI for finance_read grants and revocations (`#149a`)" | Owner-only grant and revoke of exactly `finance_read` on the existing roster row; `{ roles }` rendered and nothing else; builds on the closed #123 |
| #152 (`#149b`) | — | **Created `status:proposed`, `priority:p1`, 5, `track:design`** | A persona-neutral `/finance` route over an extracted shared component, `/pro/finance` retained; every accessible workspace with `accessMode`, no default selection among several, `finance_read` as read-only with no mutation control, cursor pagination, safe return to selection on revocation |

Both stories are children of #38, keep traceability to the closed #44 and to #111, and stay
`status:proposed` behind one gate: the canonical `design/claude-design` workspace
(`00a0f06`) carries staff grants for `practitioner_chat` only and no `finance_read` persona
screen, so each story's states must be added there in a later design turn before implementation
(`V33-DEC-037` R8). The web app's single media query and inline styling are recorded as an
implementation constraint (R9), and the missing workspace display name and `Cache-Control:
no-store` header stay disclosed follow-ups rather than silent additions (R10). #123, #111 and
#44 are unchanged and closed.

**What moved.** #149 5 → 3 (−2) and #152 created at 5 (+5): V3.3 scope 295 → **298**, done
unchanged at **225**. The live dashboard (issue #2) and an independent recomputation from raw
labels agree: **225 / 298, zero data-quality warnings**. #42, #43, #45, #47 and #99 remain
untouched; no commercial value, payment provider, deployment, tag or Release was introduced.

## V3.3 backend prerequisite of `#149a`/`#149b` ratified and created, 2026-09-12

`V33-DEC-038` closed the identification and cache-safety gaps found while reviewing the V3.3
frontend designs for #149 (`#149a`) and #152 (`#149b`) against `master` `b54bb84`. **Security,
privacy and read contract only** — no commercial value, no Legal wording, no table, column or
migration, no new ADR (ADR-049 §4.5 received one dated clarification note) and no implementation
in the governance change. The prototypes had compensated for missing server data with a fixed
placeholder on the member who receives `finance_read` and with session-local ordinals on finance
workspaces; the repository confirmed why: the staff roster projects raw identity UUIDs and no
human-readable identification while being readable by managers and staff, a NULL-linked bookkeeper
is grantable but indistinguishable, `FinanceWorkspaceEntry` carries no label, and no authenticated
finance read sends `Cache-Control`.

| Item | Before | After | Outcome it owns |
|---|---|---:|---|
| #154 | — | **Created `status:proposed`, `priority:p1`, 5, `track:engineering`** | An owner-only `GET /v1/businesses/:id/staff-management` read (professional public name or the final four phone digits, live scoped roles, no raw ids), `FinanceWorkspaceEntry.displayLabel` bulk-resolved through composition-root ports with `workspaceRef` byte-identical, and `Cache-Control: private, no-store` on all nine finance reads |
| #149 (`#149a`), #152 (`#149b`) | `status:proposed`, 3 and 5 | **unchanged**, now depending on #154 and on the narrow Claude Design correction | R13 |

**What moved.** #154 created at 5: V3.3 scope 298 → **303**, done unchanged at **225**. The live
...
## V3.3 owner policy ratification of the final 25 questions, 2026-09-12

The product owner approved **every recommended option** of the final Persian decision document
(`BeauClick-Final-Decision-List.docx`: 25 numbered questions, each with every alternative and one
recommendation, plus fifteen already-closed decisions reproduced for context). The register records
the approvals as four cards grouped by the domain they bind — `V33-DEC-039` (cancellation, no-show,
reschedule, dispute, retention, explicit acceptance; #42), `V33-DEC-040` (commission engine with
`zero` first, fee allocation by cause, revenue recognition by type, release predicate, weekly initial
schedule, reserve → future earnings → manual claim; #43), `V33-DEC-041` (provider-held money released
by outcome with split only as fallback, real-money fail-closed rules, booking credit as a non-cash
non-transferable entitlement, purchased credit non-expiring and refundable; #47, #99) and
`V33-DEC-042` (intermediary role, snapshotted legal seller by kind, responsibility split — as
published text only). The owner-facing Persian matrix is
[`V3.3_OWNER_POLICY_RATIFICATION_2026-09-12.md`](../roadmap/v3.3/V3.3_OWNER_POLICY_RATIFICATION_2026-09-12.md).
**No question awaits the owner.** No lawyer, accountant, tax adviser, bank or payment provider
approved anything, and none is claimed; those remain external facts named on #42, #43 and #47.

| Item | Before | After | Why |
|---|---|---|---|
| #42 | `status:proposed`, `gate:legal`, 13; body said `V33-DEC-013/014/017` await owner + Legal | `status:proposed`, `gate:legal`, **21**; body replaced with `V33-DEC-039` R1–R14 | Owner decision closed. The ratified contract adds a dispute case model with appeal and evidence, a no-show declaration model, a seller policy-selection surface, the customer's choice on non-customer cancellation and the explicit-acceptance write — more than one 13-point outcome; the operating model forbids `sp:21` entering Ready, so decomposition after a read-only readiness audit is the next governance step. `gate:legal` names the lawyer's opinion and copy review, not a choice |
| #43 | `status:proposed`, `gate:external`, 21 | **unchanged labels**; body replaced with `V33-DEC-040` R1–R8 | Already at the decomposition ceiling; `gate:external` names the real rail |
| #47 | `status:blocked`, `gate:external`, `gate:legal`, 13 | **unchanged labels**; body names the approved holding model and the seven exact missing external facts | The blocker now names evidence, not a choice |
| #99 | `status:blocked`, `gate:external`, 5 | **unchanged labels**; body carries `V33-DEC-041` R5–R7 and names the future refund story | No adapter exists |
| #45 | `status:proposed`, 13 | **unchanged**; dated cross-reference to the ratified UI states | Design must consume them |
| #149, #152 | `status:proposed` | **untouched** | Their design correction runs independently |

**What moved.** V3.3 scope **303 → 311** (+8, the #42 re-estimate), done unchanged at **230**;
proposed 55 → 63, blocked 18. The live dashboard (issue #2) and an independent recomputation from
raw labels agree. No issue was created or closed, no story was marked done or implemented, no
code, migration, schema, frontend, provider or credential was introduced, and no tag in the `v*`
release namespace was created. `scripts/docs-audit.mjs` now requires exactly 42 contiguous
`V33-DEC` cards.

**Disposition of what this ratification does not touch.** V3.2-A–C backend surfaces (AI
assistant, internal chat, wishlist, referral) still have no production frontend — an explicit
frontend-completion obligation whose stories are still to be created. V3.2-D (CRM/delegation)
and V3.2-E (B2B/campaigns) are not started by any of the 25 approvals. V3.2-F's payout rail stays
external (`V32-DEC-028` open; `V33-DEC-040` decides only when money is releasable). V3.2-G and
V3.4 remain evidence-gated. No scope disappears.

## V3.3 Story #42 decomposed and re-estimated, 2026-09-13

A read-only readiness audit of #42 against `57448ffd880a5977ec8d04c704764f1979126c75` returned
**NEEDS SPLIT**: the `V33-DEC-039` contract is fully ratified, nothing blocks building it in
disabled-by-absence form, and the operating model forbids `sp:21` entering Ready. The owner approved
the audit's decomposition, estimates, sequencing and technical recommendations; `V33-DEC-043` records
them and **ADR-051** (`docs/roadmap/v3/adr/ADR-051-versioned-booking-outcome-policy-and-dispute-model.md`)
was committed alone first as the family's engineering record. No product policy changed;
`V33-DEC-039`–`V33-DEC-042` stand verbatim. No lawyer, accountant, provider or bank approved anything.

| Item | Before | After | Outcome it owns |
|---|---|---|---|
| #42 | `status:proposed`, `gate:legal`, 21 | **`#42a`**: retitled, **`status:ready`**, `gate:legal`, **13** | administrator-published booking-outcome policy family (ranges/sets, Legal-evidence-gated `legalCap`) and Persian copy family; zero behaviour change |
| #159 | — | **`#42b`**: `status:proposed`, 13 | seller selection inside published ranges, immutable `commerce.order_outcome_terms` snapshot, snapshotted legal seller, explicit acceptance in the same transaction |
| #160 | — | **`#42c`**: `status:proposed`, `gate:legal`, 13 | deterministic cancellation/reschedule evaluator on the database clock; retention zero until an evidence-referenced cap is published; today's full refund preserved otherwise |
| #161 | — | **`#42d`**: `status:proposed`, 8 | no-show declaration after the snapshotted grace; the customer's full-refund/free-reschedule choice after seller/platform/provider cancellation |
| #162 | — | **`#42e`**: `status:proposed`, `gate:legal`, 13 | one dispute per booking, one appeal by a database-enforced different reviewer, hold, non-enumeration, protected evidence |
| #38 | child line "#42" | child line lists the five aliases | traceability only; no points on the epic |
| #43, #45, #47, #99, #149, #152 | — | **unchanged** | — |

**What moved.** V3.3 scope **311 → 350** (+39), done unchanged at **230**; proposed 63 → 89; ready
0 → 13; blocked 18. The live dashboard (issue #2) and an independent recomputation from raw labels
agree: **230 / 350, zero data-quality warnings**. Order `#42a → #42b → #42c → #42d → #42e`; only
`#42a` is Ready. `gate:legal` on `#42a`, `#42c` and `#42e` gates publication/activation of
Legal-dependent content (copy, `legalCap`, evidence retention), not construction of the fail-closed
control plane; `gate:external` is on none of them. The seller actor remains the currently
authorized professional session — no delegated business-owner or manager calendar authority is
introduced (`V33-DEC-030`). `scripts/docs-audit.mjs` now requires exactly 43 contiguous `V33-DEC`
