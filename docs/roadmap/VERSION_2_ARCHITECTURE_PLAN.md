...
## V2.3 Discovery, Gap Audit & Roadmap Definition

**Document status: planning/audit only. No V2.3 code, migration, or UI exists as of this writing.** Audited baseline: `master` at `9c980ab` (`v2.2.0`), matching `origin/master` exactly, working tree clean; V1/V2.0/V2.1/V2.2 tags independently re-verified byte-for-byte unchanged (`v1.0.0`→`8494c7b`, `v1.0.1`→`a8117ec`, `v2.0.0`→`67325b5`, `v2.1.0`→`d144509`, `v2.2.0`→`9c980ab`, all matching both local and remote dereferenced hashes). This section exists precisely because the task that produced it was explicit: **do not blindly implement the old V2.3 sketch** (§6/§10 above: Campaign Engine, Financial/Payout, AI-for-Professionals). Every one of those three capabilities is re-affirmed as genuinely V2.3 material below — but the *shape* of each is different from what the original sketch assumed, once measured against the real, current code rather than re-derived from the original risk analysis alone.

### Method

Two passes: (1) a full re-read of every prior planning document (this file's own §1–§17 original assessment, the "V2.2 Strategic Roadmap" section, `PRODUCT_GAP_REGISTER.md` in full, `docs/architecture/ARCHITECTURE_PROPOSAL.md`'s original data model, `BeauClick_Version_2_Roadmap.md`) to establish what was already decided and why; (2) five independent, parallel, evidence-cited codebase audits — Financial/Payout, Campaign/Pricing, AI-for-Professionals, a re-audit of every "evidence-gated"/deferred item, and a general architecture spot-check — each required to cite real `file:line` for every claim, none permitted to assume a prior document's description was still accurate. Findings below are the intersection of both passes; every claim about "what exists today" traces to a real citation gathered during pass 2, not to pass 1's documents alone.

### What actually changed since the original V2.3 sketch

The original assessment (§4.4/§4.7/§4.10, §6, §10 above) sequenced Campaign Engine, Financial/Payout, and AI-for-Professionals into V2.3 primarily on a **risk-deferral** basis — each was "genuinely new," "highest-risk," or needed "unhurried attention." That reasoning is still sound in direction, but the real codebase audit surfaced three concrete, evidence-based corrections the original risk analysis couldn't have known without measuring the actual current code:

1. **Financial/Payout is not one monolithic, gateway-blocked capability — it's two separable concerns the original sketch conflated.** "Accepting a customer's payment" (blocked on a real Zarinpal decision, unchanged, still `EXTERNAL_CONFIGURATION`) and "recording what a professional is owed and paying them" are different problems. Nothing about computing gross/discount/refund/commission/receivable per completed order requires Zarinpal to exist — WooCommerce orders are already real today (even Cash-on-Delivery orders have a real, final `WC_Order` total). And nothing about *paying* a professional requires an automated payout API either — an admin can record a real bank transfer performed outside the system and mark a ledger batch settled, exactly the way this project already handles B2B account approval and verification review (an admin-executed, audit-logged action, not an automated one). **A real, useful Financial Ledger + manually-executed Settlement workflow is buildable now, with zero payment-gateway dependency** — only a *fully automated* payout-disbursement API integration remains genuinely gateway-gated. This is a materially smaller, safer, and more valuable V2.3 scope than "Financial/Payout, deferred until a gateway exists."

2. **Campaign Engine's real risk surface is much smaller than the original "most dependency-heavy capability" framing suggested**, because the audit found only **one** WooCommerce price-mutating hook exists anywhere in this codebase today (`TierPricingEngine`'s `woocommerce_before_calculate_totals`, B2B cart only), WooCommerce's native coupon system is **100% inert** (zero code references it — a genuine blank slate, not entanglement to design around), and Loyalty/Membership's existing discount is a proven, live-verified pattern that **never touches the cart at all** — it adds a `WC_Order_Item_Fee` directly to an already-created order. The audit also found a previously-undocumented **third** order-creation path (B2B quote acceptance, `QuoteService::accept()`) that, like bookings, calls `wc_create_order()` directly and never touches the cart either. **Two of this platform's three purchase shapes (booking, B2B quote-accept) never touch the WooCommerce cart at all** — meaning a campaign engine that only ever adds an order-level fee to those two paths, exactly mirroring `MembershipDiscount`'s already-proven, already-tested pattern, can ship with **zero** cart-filter interaction and therefore zero risk of the exact displayed-vs-charged-price bug class this project has already had to fix once (in B2B, V1). This is a smaller, safer first phase than "build a general pricing-orchestration layer" — see the Pricing Orchestration Decision below.

3. **AI-for-Professionals has a concrete, previously-unidentified schema blocker, not just an authorization-engineering task.** `wp_bc_ai_conversations` carries `UNIQUE KEY (user_id)` — one conversation per WordPress user, full stop, with no `provider_id`/mode dimension. A professional who is also a customer (or who simply starts using professional-facing AI) would silently share one conversation row and one flat `ai_context` blob with their own customer-mode AI history unless this is fixed first. This is a real, concrete, cheap-to-fix prerequisite the original assessment's "mostly authorization-boundary engineering" framing didn't anticipate, because it required reading the actual migration, not the capability description.

None of this changes the destination — all three capabilities remain correctly assigned to V2.3, exactly as every prior document already concluded. What changes is the **shape**: each is smaller, more separable, and lower-risk in its first real phase than the original risk analysis assumed, once measured against what the codebase actually does today rather than derived from first principles alone.

### Codebase audit — what exists today, by capability

**Campaign/Pricing** (full citations gathered by parallel audit, condensed here):
- `TierPricingEngine` (`beauclick-b2b/src/Pricing/TierPricingEngine.php:19-20`) is the *only* price-mutating WooCommerce hook anywhere in `beauclick-*` — `woocommerce_before_calculate_totals` at priority 20, cart-scoped, B2B wholesale only.
- `MembershipDiscount` (`beauclick-loyalty/src/Pricing/MembershipDiscount.php:37-38`) hooks the custom `beauclick/booking/after_create` action (after the order already exists) and adds a `WC_Order_Item_Fee` directly — confirmed, by reading the code, to never touch `WC()->cart` in any path.
- WooCommerce's native `shop_coupon` system has **zero** references anywhere in `beauclick-*` — genuinely unused, not just unconfigured (contra the earlier gap register's "no coupons configured" framing, which implied dormant-but-wired; it's actually never wired at all).
- Three real purchase shapes exist, not two: (a) normal WooCommerce cart checkout for Shop/B2B wholesale browsing (`TierPricingEngine` applies here); (b) booking orders via direct `wc_create_order()` (`BookingOrderBridge::create_order_for_booking()`, `beauclick-payments/src/Booking/BookingOrderBridge.php:15-32`) — never touches the cart; (c) B2B quote acceptance, also direct `wc_create_order()` (`QuoteService::accept()`, `beauclick-b2b/src/Business/QuoteService.php:101-118`) — never touches the cart, and today has **no discount hook of any kind** applied to it.
- No audience-segmentation/cohort service exists anywhere (`MetricsService` is aggregate-only — counts and sums, never a user-id set a campaign could target). `EarningRules` has no "first booking"/"returning customer" branching to reuse.
- `wp_bc_loyalty_tiers`/`wp_bc_loyalty_benefits` (admin-configured table + typed JSON `config` + an explicit functional-type allowlist, `Tiers/TierService.php`, `Benefits/BenefitService.php:9-33`) is a directly copyable precedent for how a campaign's own rules should be configured — not hardcoded PHP.
- `beauclick-referral`'s eligibility/idempotency/fraud-prevention pattern (self-referral blocked by construction, `UNIQUE(referee_user_id)`, race-safe status-guarded `UPDATE`, reward idempotency via a `UNIQUE(reference_type, reference_id, reason)` index feeding the one shared loyalty ledger) is a directly copyable pattern for "exactly one reward per qualifying event," though it only ever issues loyalty points, never touches price.

**Financial/Payout:**
- A full-tree, case-insensitive search for `commission|payout|earning|settlement` across every `beauclick-*` plugin and the React app returns **zero hits** for the professional-payout concept (only unrelated customer *loyalty-points* "earning" matches) — the "zero existing foundation" characterization is accurate for this specific concept, confirmed rather than assumed.
- Two live, append-only, `reference_type`/`reference_id`-shaped tables already establish exactly the schema pattern a financial ledger needs: `wp_bc_events` (`beauclick-core/src/Database/Migrations/CreateEventsTable.php:31-43`) and `wp_bc_loyalty_points` (`beauclick-loyalty/src/Database/Migrations/CreateLoyaltyPointsTable.php:31-41`, whose own docblock explicitly calls out matching `wp_bc_events`'s audit-trail shape).
- The professional dashboard's `درآمد` (Revenue) tab is a real, honest, documented placeholder — `app/src/mounts/dashboard-professional.tsx:36` (`ready: false`), with a docblock stating outright it depends on "the future Financial/Payout system (V2.3)." The Overview tab's existing revenue figure (`DashboardController::stats()`, `beauclick-booking/src/Rest/DashboardController.php:59-77`) is **gross** order value only — summed directly from `wc_get_orders()` — never a commission-adjusted net figure, and `AnalyticsTab.tsx:68` explicitly documents that it deliberately never claims to show one.
- **Two genuinely new, previously-unflagged findings, material to Financial's real scope:** (1) `BookingService::cancel_booking()` (`beauclick-booking/src/Booking/BookingService.php:132-186`) triggers **no refund logic of any kind** today — cancellation only updates booking/slot status and sends mail; the only `wc_create_refund()` call anywhere in the codebase is one narrow edge case (`Payments\Plugin::handle_paid_but_unconfirmable_booking()`, `src/Plugin.php:181-208`), not a general cancellation-refund path. This is a real financial-correctness gap no prior gap-register entry named. (2) `MembershipDiscount::apply()` computes its percentage discount in float with `round(..., 2)` (`MembershipDiscount.php:61`) despite this codebase's otherwise-consistent integer-Toman convention (Toman has no subunit) — a small, real inconsistency worth correcting when Campaign's own discount-fee logic is built alongside it, so the pattern isn't propagated a second time.
- Reschedule (`RescheduleService::reschedule()`, `beauclick-booking/src/Booking/RescheduleService.php:120-287`) is confirmed, by its own explicit docblock and by code trace, to never touch the linked WooCommerce order at all — no double-order risk for Financial to worry about.
- No `.env`/`.env.example` key exists for HPOS override; WooCommerce 9.9.7 defaults new installs to HPOS-authoritative, and every BeauClick order-touching call site already uses the storage-agnostic `WC_Order`/`wc_get_order()`/`wc_create_order()` object API — Financial work inherits this safely by construction, no new discipline required.

**AI for Professionals & Businesses:**
- Today's AI is 100% customer-only — `AssistantController` (`beauclick-ai/src/Rest/AssistantController.php:13-44`) has exactly one `permission_callback` shape (`bc_use_ai_assistant`), no parameter or branch distinguishing customer vs. professional context anywhere.
- Context assembly (`AssistantService::send()`, `beauclick-ai/src/AssistantService.php:121-209`, `CatalogContext::summary()`, `beauclick-ai/src/CatalogContext.php:26-164`) reads only public catalog data (published providers/services/products) — confirmed by reading the actual query code, not the docs, that no booking/CRM/review/chat table is referenced anywhere in `beauclick-ai`.
- `JourneyContextProvider::infer_ai_defaults()` (`beauclick-journey/src/.../JourneyContextProvider.php:39-71`) is confirmed, by reading the actual return-array construction (not trusting the docblock alone), to never include the Beauty Journey profile's free-text `notes` field — the "notes never reach AI" boundary is real and current.
- `wp_bc_ai_conversations` carries `UNIQUE KEY (user_id)` (`beauclick-ai/src/Database/Migrations/CreateAiTables.php:39`) — the concrete schema blocker described above.
- Two directly-reusable, already-live ownership patterns exist for the exact authorization problem AI-for-Professionals needs solved: CRM's `is_customer_of($provider_id, $customer_id)` (`beauclick-booking/src/Crm/CrmService.php:138-145` and re-checked inside every mutating method, not just the controller) and Analytics' already-ownership-scoped `MetricsService::for_provider()` (`beauclick-analytics/src/Metrics/MetricsService.php:217-335`, resolved exclusively via `ProviderLookup`/`StaffService`, never a client-supplied id).
- `AssistantService::validate_recommendations()` (`AssistantService.php:254-285`) checks catalog existence/visibility only, never entity *ownership* — correct for today's public-catalog-only recommendations, and identified as the exact extension point that needs an ownership check the moment a recommendation could reference a provider-scoped entity (a CRM customer, a note).
- `RoleManager::professional_capabilities()`/`business_capabilities()` (`beauclick-core/src/Roles/RoleManager.php:89-114`) already `array_merge`s in `bc_use_ai_assistant` — professionals can already use the *customer* AI today. A new `bc_use_professional_ai` capability fits the exact same, already-versioned `maybe_register()` machinery cleanly.

**Evidence-gated / previously-deferred items — re-audited, not assumed:**
- **Notification bell/center (NOTIF-04):** unchanged — still a plain read-only list (`NotificationsList.tsx`, whose own docblock says as much), customer-only, no bell/unread-count component anywhere in `app/src`. Remains evidence-gated; Step 11/16's analytics foundation now exists to actually measure engagement with the current simple list before justifying a heavier UI, which nothing in this audit did (out of scope for a planning-only pass).
- **Marketplace fuzzy search (MKT-02) — reframed, not just re-confirmed:** the real gap is bigger than "not fuzzy" — `MarketplaceController::browse()` (`beauclick-marketplace/src/Rest/MarketplaceController.php:67-134`) has **no free-text query parameter of any kind**, confirmed by an explicit code comment stating exactly that. Only structured filters (city/specialty/price/rating) exist. This is a materially different, and materially cheaper-to-fix, gap than "typo-tolerant search," and a real omission for a marketplace product — a plain, indexed, `LIKE`-based name/bio search (matching CRM's own already-proven Persian-digit-normalized text-filter pattern) is a small, safe, high-visibility addition, distinct from and prerequisite to ever justifying fuzzy/typo-tolerant matching later.
- **Professional portfolio (PROF-03):** confirmed worse than "still a placeholder string" — there is no upload endpoint, no upload UI, and no portfolio tab in the professional dashboard nav at all (only an admin-facing CPT and a public-facing empty-state template). Real, but not core to V2.3's monetization theme — recommended for V2.4, not V2.3 (see below).
- **B2B quote-request/accept UI:** confirmed still completely missing on the frontend (`page-b2b.php` has zero references to quotes; only the wholesale direct-purchase flow exists), despite the backend (`B2BController`'s `/b2b/quotes*` routes, `QuoteService`) being complete and live since V2.1. A real, contained, low-risk, high-value gap — directly serves V2.3's monetization theme (B2B is a real revenue channel with a finished backend nobody can reach).
- **Professional notification-preferences UI (NOTIF-07):** confirmed still customer-only (`AccountTab.tsx` is the only consumer of the already-generic `PreferenceService`/`NotificationsController`) — a small, contained, low-risk addition.
- **Staff model granularity (PROF-07):** confirmed still exactly as narrow as documented — one hardcoded `'staff'` role literal (`StaffService::add()`), wired only into CRM and Analytics; booking actions, waitlist, availability, and review moderation all remain owner-only with no staff fallback. No new evidence of urgent need found — remains evidence-gated, **with one new consideration**: if Financial's Revenue tab ships, the default must be owner-only visibility (not automatically extended to staff) unless a real business decision says otherwise, since staff scope has never included financial data of any kind.
- **A genuinely new, previously-unflagged finding:** `wp_bc_admin_audit_log` coverage (built in Step 13) is real for every wp-admin-driven admin action, but the live REST endpoints `POST /b2b/accounts/{id}/approve` and `/reject` (`beauclick-b2b/src/Rest/B2BController.php:61-66,169-177`, admin-permission-gated, reachable independently of the wp-admin page) call `BusinessAccountService::approve()/reject()` directly with **no** `AuditLogger` call — a second, real, reachable path to the same action that bypasses the audit trail. Small, contained fix.
- **A second genuinely new, previously-unflagged finding:** the customer dashboard has its own `ready: false` placeholder — a "wishlist" tab (`app/src/mounts/dashboard-customer.tsx:18`) — not previously named in any gap register entry. Real, but low-severity and unrelated to V2.3's monetization theme.
- **A third genuinely new, previously-unflagged finding (low severity, informational):** roughly ten real `EventLogger` call sites across booking, loyalty, chat, reviews, B2B, payments, and journey (e.g. `membership_cancelled`, `message_sent`, `review_submitted`, `b2b_quote_requested/accepted`, `goal_created`) are logged but never read by `MetricsService` — genuine, harmless documentation/instrumentation drift beyond the two cases (`waitlist_joined`, `membership_activated`) the gap register already named. No action required; noted so a future analytics pass doesn't mistake "logged" for "already visible somewhere."

**General architecture (confirmed, not assumed):** 16 active plugins (the audited-for list of 15 plus `beauclick-locations`, present and load-bearing but omitted from the original 15-plugin brief), ~40 `wp_bc_*` tables, ~105 REST routes (every one behind a real `permission_callback`, enforced at registration time by `RestController::route()`), 10 WP-Cron jobs, two live AI provider adapters (`RuleBasedProvider`, `AnthropicProvider` — the latter's own docblock notes it is untested against the real API in this environment due to Iranian-IP provider access restrictions, a genuinely new, real-world external-configuration risk worth naming for V2.3/production planning). A targeted spot-check of five REST controllers across five different plugins (booking/CRM, booking/dashboard, loyalty, privacy, marketplace/verification) found consistent, correct ownership enforcement in every one — no new authorization gap discovered anywhere outside the two items named above.

### Pricing Orchestration Decision (closes COM-05 for real)

Per the V2.2 plan's own explicit placement ("design the orchestration layer as the first, explicit sub-step of V2.3's Campaign Engine work"), this is decided now, informed by the real audit above rather than deferred further:

**Campaign discounts, in their first V2.3 phase, apply only as order-level `WC_Order_Item_Fee` additions on the two direct-`wc_create_order()` paths (booking, B2B quote-acceptance) — never as a WooCommerce cart filter, and never touching Shop/B2B-wholesale cart-based browsing purchases at all in this phase.** This is not a compromise; it is the smaller, safer, and — given both real purchase shapes needing a promotion mechanism today are the two paths already outside the cart — actually *sufficient* architecture, not merely a cautious first step. Fee-application order on a single order is fixed and explicit: **Membership discount fee first, Campaign discount fee second**, both computed from the order's pre-discount total independently (no compounding on top of each other's discount), both logged separately for the ledger to read distinctly. A future phase 2 (cart-based Shop-promotion support) is explicitly out of this document's V2.3 scope and should only be designed if real evidence shows Shop/B2B-wholesale promotions matter — the same evidence-gating discipline already applied to Notification Center and Fuzzy Search above.

### Financial Truth — the concepts, separated (per the task's own explicit instruction not to invent "professional earnings" from gross order value)

| Concept | Definition | Buildable now? |
|---|---|---|
| Gross Order Value | `WC_Order::get_total()` — already real, already read (Overview tab) | Yes — already exists |
| Discount | Sum of Membership + Campaign order-level fees (negative) already applied to that order | Yes — both mechanisms already exist/are being built in Step 17 |
| Refund | Amount actually refunded via `wc_create_refund()` against that order | Partially — mechanism exists for one edge case only; general cancellation-refund logic is a real, newly-identified gap this plan assigns to Step 18 |
| Platform Commission | `(Gross − Discount − Refund) × commission_rate` — `commission_rate` is `NEEDS_BUSINESS_DECISION`, configured admin-side (mirroring `wp_bc_loyalty_tiers`' own "admin-configured, never hardcoded" convention), filterable, with an explicit provisional engineering default in the same spirit as `ReferralConfig`'s own documented-provisional constants | Yes, once the rate exists as configuration — the *computation* needs no gateway |
| Professional Receivable | `Gross − Discount − Refund − Commission` for that order, an append-only ledger row | Yes — pure computation over data that already exists |
| Settlement (batch) | A grouping of Receivable rows into a payout batch for one professional/business over a period, admin-reviewed | Yes — a workflow, not a payment |
| Payout | The batch marked `paid` by an admin, recording that a real bank transfer was performed **outside this system** | Yes, as a manual, admin-executed, audit-logged action — exactly like B2B account approval already works |
| Net (automated disbursement) | An API-driven, automatic bank transfer triggered by this system | **No** — genuinely requires a real payment/payout-gateway decision and credentials; correctly `EXTERNAL_CONFIGURATION`-gated, not part of any V2.3 step |

This table is the single most consequential reframing this planning pass makes: **everything through "Payout" is buildable in V2.3 with zero payment-gateway dependency.** Only fully-automated disbursement remains blocked, and it was never realistically going to ship in V2.3 regardless (no gateway is configured, and automating real money movement before manual settlement has even been exercised once would be reckless). This does not weaken the original "Financial/Payout is the single highest-risk capability" framing — real money and real audit correctness remain genuinely hard to get right — it makes the *scope* correct instead of needlessly gated on an unrelated external dependency.

### V2.3 recommended Steps

#### Step 17 — Pricing Orchestration + Campaign Engine (Phase 1: Order-Level Promotions)
- **Objective:** Ship the smallest real campaign mechanism that lets the business run actual promotions (percentage/fixed discount, date-bounded, usage-limited, simple eligibility) against booking and B2B-quote orders, without touching the WooCommerce cart or WooCommerce's own coupon system, and without needing a segmentation service that doesn't exist.
- **Dependencies:** None blocking. Loyalty/Membership (done, V2.1 Step 9), B2B (done, V1/V2.1), Analytics (done, V2.2 Step 11) all already exist to integrate against or borrow patterns from.
- **Database impact:** New `wp_bc_campaigns` (id, name, type[`percentage`|`fixed`], value, scope[`booking`|`b2b_quote`], starts_at, ends_at, usage_limit, per_customer_limit, eligibility rules as typed JSON — mirroring `wp_bc_loyalty_benefits`' `source_type`/`config` shape — is_active, priority, created_by) and `wp_bc_campaign_redemptions` (campaign_id, order_id, customer_id, applied_at), unique-constrained against double-application per order, mirroring referral's own idempotency discipline.
- **API impact:** New admin authoring/list/activate-deactivate endpoints; a server-side-only "eligible campaigns for this order" resolution called from the same `beauclick/booking/after_create`-style hook `MembershipDiscount` already uses, never a client-supplied campaign id.
- **UI impact:** A new admin campaign-builder page (existing admin-page pattern); customer-facing promotion surfacing at checkout/Journey (existing surfaces, not a new subsystem).
- **Eligibility scope, deliberately minimal:** date range, service/professional/specialty targeting (already-real fields on `wp_bc_provider_index`/`wp_bc_bookings`), "first booking ever" and "returning customer" (new, simple, direct queries against `wp_bc_bookings` — no segmentation service required), active-membership-tier targeting (already-real). Explicitly excluded: any audience concept requiring precomputed cohorts/segments, since no such infrastructure exists and building one speculatively here would repeat the exact premature-infrastructure mistake this project's own standing discipline warns against.
- **Explicitly excluded:** WooCommerce coupon integration (genuinely unused today — a blank slate not worth entangling with a new system); any cart-based/Shop-browsing promotion (phase 2, evidence-gated); discount stacking beyond the fixed Membership-then-Campaign order defined above.
- **Security impact:** Campaign definitions are admin-owned (not per-professional), so the ownership model is simpler than CRM's — but eligibility resolution must be 100% server-side, never trusting a client-supplied campaign id or discount amount, exactly like every other price-affecting code path in this codebase.
- **Complexity:** Medium — the mechanism is small; the discipline of never touching the cart is what keeps the risk small too.
- **Why V2.3, not later:** Directly serves V2.3's monetization/growth theme, and the exact reason it was previously deferred (an uncoordinated third price hook) is resolved by this step's own scope decision, not by waiting further.
- **Definition of done:** A real, admin-authorable campaign with working eligibility, usage limits, and the fixed Membership→Campaign fee order; zero cart-filter code added anywhere; a dedicated regression test proving displayed price equals charged price for a stacked Membership+Campaign booking (the exact bug class this project has already shipped once); full Persian/RTL/Jalali.

#### Step 18 — Financial Ledger & Manual Settlement
- **Objective:** Give professionals and businesses a real, commission-adjusted earnings figure for the first time (closing the `درآمد` tab's placeholder), and give admins a real, audit-logged way to record that a professional has been paid — entirely without depending on an automated payment-gateway integration, per the Financial Truth table above.
- **Dependencies:** None blocking on Step 17 (the ledger reads the order's own final total and fee line items generically — it does not need to know *which* feature added a given fee, so no ordering dependency exists, only a data-shape one both steps already share). Requires one business decision before it can compute anything real: the commission rate (provisional, filterable default acceptable, same convention as `ReferralConfig`).
- **Database impact:** New `wp_bc_ledger_entries` (append-only: id, entry_type[`gross`|`discount`|`refund`|`commission`|`receivable`], reference_type, reference_id, provider_id/business_id, amount, created_at) — directly modeled on `wp_bc_events`/`wp_bc_loyalty_points`' own proven shape. New `wp_bc_payout_batches` (id, provider_id/business_id, period_start, period_end, total_amount, status[`pending`|`paid`], paid_at, paid_by_admin_id, note) — payout status kept structurally distinct from order status, per the original architecture assessment's own explicit requirement (§4.4 above), never conflated.
- **API impact:** Read-only self-scoped earnings endpoint (reuses the `ProviderLookup`/`StaffService` ownership-resolution pattern already proven by Analytics and CRM); admin-only batch-creation and mark-paid endpoints. No ordinary-user write path to any ledger row, matching this codebase's own standing "financial records must never be editable through ordinary interfaces" principle (§8 above, re-affirmed).
- **UI impact:** The real `درآمد` tab (professional/business dashboard) replacing its `ready: false` placeholder; a new admin Financial/Payouts page (existing admin-page pattern) for batch review and mark-paid.
- **A real, newly-discovered gap this step must also close:** wire actual refund logic into `BookingService::cancel_booking()` (reusing the one existing `wc_create_refund()` precedent, gated on the order actually being paid, not still-pending COD) — without this, "Refund" in the Financial Truth table has no real writer, and the ledger's own correctness depends on it.
- **Explicitly excluded:** Any automated payout/disbursement API integration (`EXTERNAL_CONFIGURATION`-gated, not scheduled); any change to how commission is *decided* (business decision, not this step's to make); staff visibility into financial data (defaults to owner-only, per the Staff Model finding above, unless a real business decision opens it).
- **Security impact:** The single highest-scrutiny step in V2.3 — immutable ledger rows (compensating entries only, never `UPDATE`/`DELETE`), full audit trail via the existing `AuditLogger` pattern (and the B2B REST-audit-log gap found during this audit should be fixed here too, as a template for "every money-adjacent action must be logged on every reachable path, not just the wp-admin one"), explicit adversarial test coverage for cross-provider/cross-business ledger isolation.
- **Complexity:** High — not because the mechanism is exotic (it directly reuses two already-proven table shapes), but because financial correctness under refunds/cancellations/reschedules is genuinely hard to get exhaustively right, exactly as the original assessment warned.
- **Why V2.3, not later:** The `درآمد` tab has been a reserved, empty placeholder since V1 — this is the single most-referenced "coming in V2.3" promise across every prior planning document, and this audit confirms it is buildable now without the gateway dependency previously assumed to block it entirely.
- **Definition of done:** A real, correct, test-covered ledger computing gross/discount/refund/commission/receivable per completed order; a real refund path wired into cancellation; a real, admin-executed settlement/mark-paid workflow with a full audit trail; the professional/business dashboard showing a real net figure for the first time; explicit test coverage proving one provider's ledger data is never visible to another; full Persian/RTL/Jalali (Toman formatting, integer money math throughout — including fixing Step 17/this step's own campaign and membership discount math to the platform's integer convention).

#### Step 19 — AI for Professionals & Businesses (Read-Only Insights)
- **Objective:** Give professionals/businesses their first AI-generated insight into their own CRM/booking/analytics data — strictly **read-only insight**, never a recommendation the AI acts on and never an autonomous action, per the task's own explicit READ-ONLY INSIGHT / RECOMMENDATION / ACTION distinction. Example scope: "customers who haven't returned in 60 days," "your top-performing service this month," narrated in Persian from real, already-owned data — not a chat-driven agent that can message a customer or apply a discount on the professional's behalf.
- **Dependencies:** None blocking (CRM since V2.1 Step 5, ownership-scoped Analytics since V2.2 Step 16 both already exist and are directly reusable). One self-contained prerequisite sub-step: fix the `wp_bc_ai_conversations` schema gap first (see below) before any professional-mode AI code is written against it.
- **Database impact:** A new, separate `wp_bc_ai_professional_conversations`/`_messages` pair (not a modification to the existing, working, live `wp_bc_ai_conversations` table or its `UNIQUE(user_id)` constraint) — matching this codebase's own repeated "extend, don't touch what already works" discipline (see Steps 8/13's own stated reasoning for the same choice). This avoids any risk to the existing, proven customer AI path while giving professional-mode AI its own clean `provider_id`-scoped identity.
- **API impact:** A new, separate professional-scoped route (not a `context` parameter grafted onto the existing customer `/ai/messages` endpoint, for the same reason as above) reusing `AssistantService`'s existing guard/rate-limit/provider-abstraction machinery, but with its own context-assembly function built from `CrmService`'s `is_customer_of()` and `MetricsService::for_provider()` — never a second, parallel query path.
- **UI impact:** A new AI entry point inside the professional/business dashboard (distinct tone/data from the customer-facing panel).
- **Security impact:** The single most important requirement, restated because the original assessment named it and the audit confirms it's still the crux: a professional's AI session must only ever assemble data belonging to *that* professional. `validate_recommendations()`'s existing DB-existence check must gain an explicit ownership check the moment any recommendation type could reference a CRM-scoped entity — this needs its own, explicit, adversarial test (a second professional's session attempting to read the first's insight), not just happy-path coverage, mirroring the exact test discipline already used for CRM's own cross-provider isolation.
- **New capability:** `bc_use_professional_ai`, added to `professional_capabilities()`/`business_capabilities()` (not `customer_capabilities()`), using the exact same versioned `maybe_register()` machinery every other capability in this codebase already uses.
- **Explicitly excluded:** Any autonomous action (sending a message, applying a discount, editing a booking) — strictly read-only narration of already-owned data in this phase; any AI-driven segmentation/targeting feeding Campaign (Step 17) — that would be a real, separate future integration, not assumed here; cost/usage visibility (AI-03, still correctly deferred until a real paid provider is actually configured — unchanged).
- **Complexity:** Medium — mostly the same authorization-boundary engineering the original assessment predicted, plus the newly-discovered schema fix.
- **Why V2.3, not later:** CRM and ownership-scoped Analytics — its two real dependencies — have both existed since V2.1/V2.2; nothing more needs to be built first. Kept last in this plan's own recommended sequencing (not blocked, just lower-urgency) because getting the authorization boundary right deserves the same unhurried, dedicated attention the original assessment already argued for.
- **Definition of done:** A real, working professional-facing AI surface reading only that professional's own CRM/analytics data, strictly read-only insight, with an explicit passing adversarial cross-professional-isolation test; the schema fix landed and tested before any professional-mode message is ever sent; full Persian/RTL.

#### Step 20 — Growth & Professional Platform Quick Wins
- **Objective:** Close a set of small, independently-shippable, high-value-per-effort gaps this audit re-confirmed or newly discovered — none large enough to deserve its own step, all real.
- **Dependencies:** None — fully parallelizable with Steps 17–19 and with each other.
- **Scope, itemized:**
  1. **B2B quote request/accept UI** — a real frontend for the already-complete, already-live `/b2b/quotes*` backend (request, list-own, accept, admin quote-back). No new backend work; this is a pure UI gap closure.
  2. **Basic marketplace text search** — a plain, indexed, `LIKE`-based professional/service name and bio search on the marketplace, reusing CRM's own already-proven Persian/Arabic-digit normalization pattern. Explicitly **not** fuzzy/typo-tolerant matching (still correctly evidence-gated past V2.3, per the original MKT-02 reasoning) — this closes the actually-larger gap this audit found (no free-text search existed at all), which is a prerequisite to ever meaningfully evaluating whether fuzzy matching is worth its own future step.
  3. **Professional/business notification-preferences UI** (NOTIF-07) — a new dashboard surface for the already-generic `PreferenceService`/`NotificationsController`, mirroring the existing customer `AccountTab.tsx` implementation exactly.
  4. **Admin audit-log REST gap fix** — add the missing `AuditLogger` call to `B2BController`'s REST-driven approve/reject actions, so the audit trail is complete on every reachable path, not only the wp-admin one.
- **Explicitly excluded from this step:** Portfolio upload (real, but not core to V2.3's monetization theme — recommended for V2.4, see below), staff permission granularity expansion (remains evidence-gated), the customer-dashboard wishlist placeholder (real but low-severity, unrelated theme — V2.4 candidate), fuzzy search (still evidence-gated past this step).
- **Complexity:** Low across all four items — this is intentionally the cheapest, fastest step in V2.3, suitable to ship first as a confidence-building start to the release.
- **Why V2.3, not later:** Items 1 and 3 both close gaps in already-complete backends nobody can reach — the cheapest possible way to unlock real value already built. Item 2 is a materially-sized, previously-mischaracterized product gap (no search box, not "no fuzzy search"). Item 4 is a small, correctness-relevant fix best made now rather than left open indefinitely.
- **Definition of done:** All four items real, tested, live-verified, full Persian/RTL/Jalali/mobile.

### Dependency graph

```
Step 17 (Campaign, Phase 1)         ── independent; only internal prerequisite is its own
   │                                   Pricing Orchestration Decision (already made above)
   │
Step 18 (Financial Ledger)          ── independent; only internal prerequisite is the
   │                                   commission-rate business decision (provisional default
   │                                   acceptable); reads any order's final fees generically,
   │                                   so it has no build-order dependency on Step 17
   │
Step 19 (AI for Professionals)      ── independent; only internal prerequisite is its own
   │                                   ai_conversations-schema-separation sub-step (self-contained)
   │
Step 20 (Growth & Platform          ── fully independent, four unrelated small items,
         Quick Wins)                   zero dependency on 17/18/19 or on each other
```

**The single most consequential structural finding of this planning pass: unlike V2.1/V2.2 (where steps had real, sequential dependencies — Auth before Legal before Verification; CRM before AI-for-Professionals; Analytics before Step 16's analytics consumption), none of V2.3's four steps blocks any other.** Each has exactly one small, self-contained internal prerequisite (a decision or a schema fix), not a cross-step dependency. This means V2.3 can be resourced as four genuinely parallel workstreams if capacity allows, or run sequentially in any order without rework risk.

### Critical path and parallelizable work

**Critical path:** none, in the blocking sense — no step's start depends on another step's completion. If sequenced for risk/value reasons rather than technical necessity, the recommended order is:

1. **Step 20** first — cheapest, fastest, unlocks value already built and paid for (B2B backend, notification backend), builds release confidence before the higher-stakes steps.
2. **Step 17** — moderate complexity, directly enables real monetization campaigns, and its Pricing Orchestration Decision is already made in this document rather than left as in-flight risk.
3. **Step 18** — the highest-scrutiny step (real money, real audit correctness) — sequenced after 17 only so the fee-application order it must account for is already a settled fact, not a moving target, even though no hard technical dependency exists.
4. **Step 19** — kept last, matching the original assessment's own "deserves unhurried, dedicated attention" reasoning, unchanged by this audit.

**Parallelizable work:** all four steps, in full, if resourced separately — there is no shared file, table, or hook contention between them (Step 17 and Step 18 both read `WC_Order` fee data but neither writes to a structure the other owns).

### V2.3 Definition of Done

Every V2.3 step (17 through 20) must satisfy the same standing bar unchanged since V1/V2.0/V2.1/V2.2: real business logic, real permissions/ownership enforcement (server-side, never frontend-trusted), real validation, loading/empty/error states, complete Persian RTL UI with correct Jalali dates via the existing shared implementation, verified desktop and mobile (375/390/412) behavior, real test coverage for every new ownership boundary and money-adjacent path, live browser verification where applicable, a security review, updated documentation, and a real git commit. Two V2.3-specific additions to this standing bar, given what this version actually touches: (1) every money-adjacent code path (Steps 17, 18) needs an explicit test proving displayed price equals charged price and that no cross-tenant financial data leak is possible, not just happy-path coverage; (2) every AI-adjacent code path (Step 19) needs an explicit adversarial cross-professional-isolation test before shipping, not just a correctness test.

### Product experience — what V2.3 should mean, in plain terms

- **Customer:** sees real, occasionally-discounted prices on bookings and B2B-relevant purchases when a real promotion is running; can, for the first time, actually search the marketplace by typing a name or keyword instead of only filtering by city/specialty/price/rating.
- **Professional:** sees a real, honest, commission-adjusted earnings figure for the first time — the single most-anticipated "coming later" promise this product has carried since V1 — plus a first AI-generated read of their own customer/booking patterns ("who hasn't come back," "what's working"), and control over their own notification preferences.
- **Business:** everything a professional gets, plus a business buyer can finally request and view and accept a wholesale quote through the product itself, instead of the backend existing with no way to reach it.
- **Admin:** a real campaign-authoring surface; a real financial ledger and a real, audit-logged settlement workflow (mark-a-batch-paid, not automated disbursement); complete audit-log coverage across every reachable action path, not just the wp-admin one.

### What moves to V2.4+, and what remains evidence-gated

- **Portfolio upload (PROF-03):** real, confirmed, but not core to V2.3's monetization/growth theme — recommended **V2.4**, the clearest single candidate to lead that release.
- **Customer dashboard "wishlist" placeholder:** real, low-severity, unrelated theme — **V2.4**, bundle opportunistically with portfolio or other dashboard work.
- **Staff permission granularity beyond CRM+Analytics (PROF-07):** remains evidence-gated — no new evidence of urgent need found this audit; Step 18's own new "financial data stays owner-only by default" boundary is the only new consideration, and it argues for *not* expanding staff scope casually, not for building a full permission matrix now.
- **In-app notification bell/center (NOTIF-04):** remains evidence-gated — Step 11's analytics foundation (live since V2.2) can now actually answer "is anyone using the current simple list" with real data; that's a cheap query to run before ever committing engineering time to a heavier UI, but it's a measurement task, not a build task, and out of this planning-only pass's scope.
- **Fuzzy/typo-tolerant search:** remains evidence-gated past Step 20's basic-search addition — revisit only once real search-volume/failure data exists from the feature Step 20 actually ships.
- **Realtime Communication, Multi-Sided Marketplace, Native Mobile:** unchanged, still V2.4+, evidence-gated — nothing discovered during this audit changes that calculus.
- **Automated payout/disbursement API integration:** correctly excluded from every V2.3 step — genuinely `EXTERNAL_CONFIGURATION`-gated on a real payment/payout-gateway decision, and realistically not worth attempting before a manual settlement workflow (Step 18) has been exercised in production at least once.
- **Campaign Engine Phase 2 (cart-based Shop/B2B-wholesale promotions):** deliberately out of Step 17's scope — revisit only if real evidence shows Shop/B2B-wholesale promotions matter; the two purchase shapes Step 17 actually covers (booking, B2B quote-accept) are where real promotional value already exists today.

### Business decisions still required (not resolved by this plan)

- **Commission rate** (Step 18) — the single most consequential open business decision blocking real monetization; a provisional, filterable engineering default (matching `ReferralConfig`'s own precedent) is acceptable to unblock engineering, but the real rate is a business call.
- **Payout schedule/frequency and minimum payout threshold** (Step 18).
- **Refund responsibility/policy specifics** (Step 18) — ties directly to the still-open `LEGAL-03` (`بازگشت و استرداد وجه` policy specifics) from the V2.1/V2.2 gap register; should ideally be resolved together, not independently.
- **Whether staff can see financial data** (Step 18) — this plan's own default recommendation is no, pending an explicit decision otherwise.
- **Campaign discount values, usage limits, and whether Campaigns should ever be paid placements** (Step 17) — the original assessment's own explicit "must stay distinguishable from organic ranking" requirement (§10 above) applies unchanged if paid placement is ever considered.
- **Whether professional-facing AI (Step 19) should ever be a paid/premium-tier feature** — plausible per the original assessment's own monetization table (§10 above), not decided here.

### External configuration relevant to V2.3

Unchanged from the V2.2 Final Release Audit, re-confirmed rather than re-litigated by this pass: real Zarinpal credentials (still required before any real customer payment can occur — a pre-existing, V2.3-independent blocker, since Step 18's ledger/settlement work has no gateway dependency of its own), a real SMS gateway, outbound SMTP, backup, and error monitoring all remain `EXTERNAL_CONFIGURATION`/`MISSING` — this audit additionally confirms none of these even have credential *key names* present in `.env.example`, not just empty values, meaning the scaffolding for them doesn't exist yet either. One genuinely new external-configuration risk surfaced this audit: `AnthropicProvider`'s own docblock notes it has never been exercised against the real Anthropic API in this environment due to Iranian-IP provider access restrictions — worth flagging explicitly for whoever eventually configures a real AI provider in production, since it may require a different access path (proxy, regional endpoint, alternate provider) than assumed.

### Architecture evolution required for V2.3

**None beyond what's named per-step above.** Every one of Steps 17–20 extends an already-established pattern: the order-level-fee discount mechanism (`MembershipDiscount`'s own proven shape), the append-only ledger shape (`wp_bc_events`/`wp_bc_loyalty_points`), the existing REST/`RestController`/ownership pattern, the existing AI provider abstraction. No new database technology, no new caching layer, no new job-queue system, no new search engine (the basic marketplace search in Step 20 is a plain indexed `LIKE` query, not a search-engine adoption), no Redis/Kafka/RabbitMQ/Elasticsearch/microservices — matching this project's own standing "prefer incremental evolution, introduce infrastructure only against measured need" principle, re-affirmed rather than re-litigated for the fourth consecutive version.

### Risks

1. **Financial correctness under refunds/cancellations/reschedules** (Step 18) — the single highest-stakes risk in this entire plan; needs explicit adversarial test coverage, not just happy-path verification, exactly as the original assessment warned for this capability specifically.
2. **Campaign/Membership fee-stacking correctness** (Step 17) — the exact bug class (displayed price ≠ charged price) this project has already shipped and fixed once in B2B; needs a dedicated regression test, not just code review, before shipping.
3. **AI cross-professional data leakage** (Step 19) — a context-assembly bug here would be a severe privacy incident, per the original assessment's own explicit warning; needs adversarial test coverage before launch, not just correctness testing.
4. **Scope creep on Campaign eligibility rules** (Step 17) — the temptation to build real audience segmentation instead of the simple, direct-query eligibility this plan specifies; resist it the same way Analytics' own "no BI platform" discipline was maintained in V2.2.
5. **Under-scoping the B2B REST audit-log fix** (Step 20) — a small fix, but worth verifying no *other* REST-only, wp-admin-page-bypassing gap exists elsewhere before considering Step 20 complete; the audit found exactly one, but did not claim to have found every possible instance.

---

## V2.3 Step 17 — Campaign & Promotion Engine (Phase 1) Implementation Notes

**Step 17 is complete.** A new `beauclick-campaigns` plugin ships a real, admin-authored, server-resolved campaign discount mechanism for booking orders, built exactly to the Pricing Orchestration Decision and Step 17 scope defined above — no cart filter, no coupon integration, no B2B/Shop involvement in this phase, no REST surface.

**Domain and schema — two tables, not three:** `wp_bc_campaigns` (name, `discount_type`[`percentage`|`fixed`], `discount_value`, `max_discount_amount`, `status`[`draft`|`active`|`paused`|`archived`], `starts_at`/`ends_at`, `service_id`/`provider_id` targeting — `NULL` = wildcard, `customer_scope`[`all`|`first_booking`|`returning`], `min_order_value`, `usage_limit_total`, `usage_limit_per_customer`) and `wp_bc_campaign_usages` (`campaign_id`, `booking_id` `UNIQUE`, `order_id`, `customer_id`, `discount_amount`, `status`[`applied`|`released`]). The originally-sketched "Campaign Rule"/"Campaign Target" tables were deliberately collapsed into fixed columns on `wp_bc_campaigns` itself — Phase 1's targeting set is small and fixed, not a rule engine, matching the task's own "do not create unnecessary tables" instruction. `CampaignService` (`beauclick-campaigns/src/CampaignService.php`) is the sole owner of both tables; `EligibilityResolver` (`src/EligibilityResolver.php`) is pure decision logic operating on data `CampaignService` provides — independently testable, matching the `TierService`/`ReferralService` convention this codebase already established.

**Integration point:** `Pricing\CampaignDiscount` (`src/Pricing/CampaignDiscount.php`) hooks `beauclick/booking/after_create` at priority 30 — strictly after `beauclick-payments`' own order-creation callback (10) and `beauclick-loyalty`'s `MembershipDiscount` (20) — and adds a single itemized `WC_Order_Item_Fee`, exactly mirroring `MembershipDiscount`'s already-proven shape. Zero WooCommerce cart/product-price hooks registered anywhere in this plugin (asserted by a dedicated structural test, mirroring `LoyaltyIntegrationTest`'s own).

**"No compounding" pricing decision, implemented exactly as specified:** Campaign's discount is computed against the order's own `get_subtotal()` independently of whatever Membership already subtracted — a 10% membership + 15% campaign on a 1,000,000 Toman booking discounts 100,000 + 150,000 = 250,000 (final 750,000), never 15% compounded onto an already-discounted 900,000. A final safety clamp (`min($discount, (int) round($order->get_total()))`, checked against the order's *current* remaining total after Membership's own fee) guarantees the combined discount can never push the order negative regardless of how aggressively both are configured — live-verified with a 60%+60% combination clamping correctly at zero.

**B2B explicitly excluded, confirmed during implementation, not assumed:** `beauclick-b2b\Business\QuoteService::accept()` (the second direct-`wc_create_order()` path named in the audit above) fires no filter/action hook of any kind a listener could subscribe to without modifying `beauclick-b2b` itself — confirmed by reading the method, not inferred. Rather than expand this step's blast radius into a plugin Phase 1 doesn't need to touch, B2B quote orders remain entirely out of Campaign Phase 1, exactly as the original plan's own "prefer excluding B2B if not explicitly required" instruction anticipated. `TierPricingEngine`'s wholesale pricing is confirmed, by a dedicated test with a deliberately aggressive 90%-discount campaign active, to be completely unaffected.

**Stacking model:** exactly one campaign per booking (no admin-configured priority field). When more than one active campaign is eligible, the one producing the **largest discount for that specific order** wins; ties break to the lowest campaign id (oldest wins) — a natural consequence of `active_candidates()`'s own id-ascending iteration order, not a separate comparison. No stacking beyond the fixed Membership→Campaign order.

**Idempotency:** `record_usage()` performs an `INSERT IGNORE` keyed on `UNIQUE(booking_id)` *before* the fee is added — the real, race-safe guarantee, not a preceding `SELECT`. A dedicated integration test proved a genuinely important, non-obvious fact about this codebase: re-firing `beauclick/booking/after_create` for the same `booking_id` doesn't retry against the same order at all — `beauclick-payments`' own upstream handler (priority 10) has no idempotency of its own and unconditionally creates a **second, distinct** WooCommerce order every time. This plugin's `UNIQUE(booking_id)` guard is what actually matters: the second, spuriously-created order for the same booking correctly receives zero discount, and exactly one usage row ever exists for that booking no matter how many orders were created for it. This is a pre-existing fact about `beauclick-payments`, not something this step introduced or needed to fix.

**Usage release on order death:** `Pricing\UsageReleaseListener` independently hooks the same three `woocommerce_order_status_cancelled|failed|refunded` actions `beauclick-payments\Plugin` already listens to (WordPress supports multiple independent callbacks per hook), moving a usage row from `applied` to `released` without deleting it — an abandoned hold or a genuine cancellation doesn't unfairly cost a customer their shot at a limited campaign, live-verified end to end (book with a per-customer-limit-1 campaign → second booking correctly blocked → cancel the first order → third booking correctly re-discounted).

**Reschedule interaction:** confirmed by a dedicated test, not just by reading `RescheduleService`'s own docblock, that rescheduling a discounted booking leaves the campaign fee, the order total, and the usage row completely untouched — `RescheduleService` never touches the linked order at all, exactly as documented.

**Admin UI — a deliberate deviation from the task's own "if REST is required" framing, documented:** Campaign CRUD/lifecycle uses classic wp-admin + `admin-post.php`, matching `LoyaltyAdminPage`'s exact convention for an admin-authored promotional-economics domain, not a REST API. No customer-facing surface ever needs to send or receive a campaign id — campaign selection is 100% server-resolved inside `CampaignDiscount::apply()` — so no REST route, and therefore no REST attack surface, exists for this feature at all. Reuses the existing `bc_manage_platform` capability (no new capability introduced, per `RoleManager`'s own "prefer a small, understandable model" precedent) and the existing `AuditLogger` for every lifecycle action (`campaign_created`/`_updated`/`_activated`/`_paused`/`_archived`), live-verified in the real admin audit log with correct actor/timestamp/Jalali formatting.

**Two small, in-scope fixes made alongside this step, as the original plan itself anticipated:**
1. `beauclick-loyalty\Pricing\MembershipDiscount` computed its discount as `round((float) $order->get_total() * ..., 2)` — float math with 2-decimal rounding on a Toman platform with no subunit. Fixed to `(int) round((int) $order->get_subtotal() * $percentage / 100)`, matching Campaign's own integer, subtotal-based convention. Harmless in isolation before this step (no other fee existed yet when it ran), but left as-is would have been the one silently-inconsistent discount calculation once Campaign's own fee existed alongside it.
2. `beauclick-payments\Receipt\ReceiptPresenter::for_order()` called `$order->get_items()`, which defaults to `line_item` type only — a real, pre-existing gap this step's own live verification caught: the Membership discount fee has never actually appeared in this dedicated receipt view, only silently lowered the total with no line explaining why (WooCommerce's own checkout/order-received templates were never affected — they already render fees correctly). Fixed to `get_items(['line_item', 'fee'])`, benefiting both Membership's existing fee and Campaign's new one. `app/src/features/booking/ReceiptView.tsx` updated in lockstep (`quantity: number | null`, since a fee line has no meaningful quantity; negative amounts rendered with an explicit minus sign rather than a raw `-` glyph).

**Analytics:** a single `campaign_applied` event (`EventLogger`, `entity_type='campaign'`) logged only when a fee is genuinely added — not a full `created`/`activated`/`qualified`/`applied`/`discounted` set as the original sketch suggested. Admin lifecycle actions go through `AuditLogger` only, not `EventLogger` — matching this codebase's own established distinction (`AuditLogger`'s own docblock: administrative actions must never leak into the product/analytics event log) rather than following the task's suggested event list literally where it would have conflated the two. A "campaign qualified but a different campaign ultimately applied" signal was deliberately not built — with no stacking beyond single-campaign-wins, it would be a hollow, always-redundant event; reporting instead lives directly in the admin page itself (`usage_summary()`: live count + total discount granted per campaign, computed from `beauclick-campaigns`' own table, no new cross-plugin dependency on `beauclick-analytics`).

**Tests:** 44 new tests (`CampaignServiceTest`, `EligibilityResolverTest`, `CampaignDiscountIntegrationTest`, `CampaignAdminPageTest`) covering lifecycle/validation, targeting/eligibility/stacking/tie-breaks, the full real-order integration path (displayed=charged, no-compounding stacking, negative-total clamp, idempotency across spurious duplicate orders, B2B non-interference, structural cart-hook-free assertion, usage release on cancellation, reschedule non-interaction), and admin audit logging. Full backend suite: **769/769** (725 baseline + 44 new), zero regressions from the `MembershipDiscount`/`ReceiptPresenter` edits. Frontend: TypeScript clean, ESLint clean, Vitest **38/38** (unchanged — no new frontend test added for the minor `ReceiptView.tsx` null-safety change, matching its scope), production build succeeds and deploys.

**Live QA:** performed against the real local WordPress site (not just PHPUnit), using a newly-created, clearly-labeled QA admin account and a newly-created QA customer account (neither touching nor resetting the pre-existing site owner's own admin credentials, per this task's own explicit "no password-reset workarounds" instruction). Verified live and directly observed: campaign creation → draft → activate through the real admin UI; both lifecycle actions correctly recorded in the real admin audit log with correct actor/Jalali timestamp; a real booking against a real demo professional/service, with the campaign's 20% discount appearing correctly at the real WooCommerce checkout page (`تخفیف کمپین: کمپین تست زنده: -۵۰۰٬۰۰۰ تومان`, subtotal 2,500,000 → final 2,000,000), on the order-received page after real COD "payment," and in the dedicated BeauClick receipt modal (confirming the `ReceiptPresenter` fix). **Known limitation, disclosed rather than fabricated:** a dedicated 375px mobile-viewport re-verification of the admin campaign page could not be completed — the browser automation pane became unresponsive mid-session after the desktop verification above was already captured. The campaign admin table is wrapped in the same `overflow-x:auto` container every other BeauClick admin page already uses (verified mobile-safe in the V2.2 Step 13 audit); the customer-facing checkout/receipt changes reuse existing, already-mobile-verified layout with no new CSS. This is a real gap in this step's own verification, named explicitly rather than claimed as done.

**No other item in this register or plan changed status as a result of this step.** Steps 18, 19, and 20 were not started. No `v2.3.0` tag was created. The rest of this document's V2.1/V2.2 completion notes and the V2.3 planning section above remain the accurate historical record and are preserved exactly as written.

---

## V2.3 Step 18 — Financial Ledger & Manual Settlement Implementation Notes

**Step 18 is complete.** A new `beauclick-financial` plugin gives BeauClick its first real, auditable record of platform commission and professional/business receivables — computed from real, already-existing WooCommerce order/refund facts, never a second pricing engine and never automated payout — built exactly to the Financial Truth table and Step 18 scope defined above.

**Domain and schema — three tables, deliberately not the four the task's own candidate list named:** "Financial Entry" was not built as a table separate from "Financial Ledger" — each row of `wp_bc_ledger_entries` already IS one entry. That table stores only `commission` and `receivable` rows (plus their negative refund-reversal counterparts) — it deliberately never stores gross/discount/subtotal, since those already exist forever, unmodified, on the real WooCommerce order/order-items (task §6's own instruction). `wp_bc_settlement_batches`/`wp_bc_settlement_items` (a settlement record + its per-order line items) complete the three. Every ledger row captures its own `commission_rate`/`basis` at write time, never looked up live — a later change to the platform's commission configuration can never silently rewrite the financial meaning of a past transaction (closing the task's own §17 "effective_at" concern without a dedicated column).

**Scope decision, confirmed during implementation, not assumed:** the ledger only ever records rows for **booking orders** (an order carrying `_bc_booking_id` meta). Reading `MyAnalyticsController::b2b_section()`'s own existing "gross order value, never earnings" framing confirmed a B2B quote-accept or Shop purchase is a **direct platform sale** — BeauClick itself is the seller, with no professional/business party to split revenue with — so those orders correctly produce zero ledger entries, verified by a dedicated test (`test_a_shop_order_produces_no_ledger_entries`). This is the same practical scope Campaign Phase 1 (Step 17) arrived at, reached independently for a different, equally valid reason.

**Double-entry decision (task §8), explicit:** full double-entry accounting (a chart of accounts, balanced debit/credit journal entries) was deliberately NOT built — not justified by a marketplace commission/receivable ledger's real need. The chosen model is the append-only, typed transaction ledger + settlement-record combination the task itself named as a valid Phase 1 answer: every entry is a signed fact tied to one order and one party (`platform` for commission, `professional`/`business` for receivable), `UNIQUE(entry_type, reference_type, reference_id)` is the real idempotency guarantee (mirrors `wp_bc_campaign_usages`' own `UNIQUE(booking_id)` discipline from Step 17), and "outstanding" is always computed fresh — `receivable net minus non-reversed settlement items` — never stored as a cached balance that could drift from the ledger.

**Commission basis and rate (task §17/§18):** `CommissionConfig` holds a provisional, filterable default (15%, `beauclick/financial/commission_rate`) in the exact same spirit as `ReferralConfig`'s own documented-provisional constants — the real rate is explicitly `NEEDS_BUSINESS_DECISION`, never invented as final policy. The basis is fixed to `net_customer_amount` — the real, already-Campaign/Membership-discounted `WC_Order::get_total()` a customer actually paid, verified end-to-end by a dedicated test with both Step 17's Campaign and Step 9's Membership discount stacked on the same order (`test_commission_is_computed_on_the_real_discounted_total_with_campaign_and_membership_stacked`): commission is 15% of the real 750,000 the customer paid, never 15% of the original 1,000,000 subtotal.

**FIN-02 closed — real refund logic wired into cancellation, exactly as this document's own Step 18 entry committed to:** `BookingService::cancel_booking()` gained one new, generic hook (`beauclick/booking/cancelled`), fired unconditionally alongside the existing `slot_opened` hook — `beauclick-booking` still doesn't know `beauclick-payments`/WooCommerce exists, matching every other hook in that file. `beauclick-payments\Plugin::on_booking_cancelled()` is the new listener: refunds the order's own real `get_remaining_refund_amount()` (never a value this method computes independently) if, and only if, the order `is_paid()`. Live-verified, not just unit-tested: cancelling a real, paid, already-settled booking issued a genuine WooCommerce refund (order status → `refunded`, `get_total_refunded()` = the full real amount), which the ledger's own `RefundRecorder` — hooking WooCommerce's own core `woocommerce_order_refunded` action independently — correctly reversed to a net-zero receivable.

**A genuinely interesting, unplanned interaction this step's own tests surfaced and confirmed correct:** re-firing `beauclick/booking/after_create` for the same real booking (the same pre-existing `beauclick-payments` fact Step 17's own test already found — that plugin's order-creation handler has no idempotency of its own, always creating a second, distinct order) causes the SECOND order's `payment_complete()` to hit the *pre-existing*, Step-18-independent `handle_paid_but_unconfirmable_booking()` path (the booking is already confirmed by the first order), which auto-refunds the second order. This step's own `RefundRecorder` correctly reacts to that real refund and nets the second order's ledger rows back to exactly zero — the ledger self-heals a scenario it was never specifically designed around, purely by correctly reacting to real WooCommerce hooks (`test_a_second_spurious_order_for_the_same_booking_still_records_its_own_correct_entries`).

**Refund-after-settlement (task §22), live-verified, not only unit-tested:** cancelling a booking whose order had **already been fully settled** through the admin UI produced the expected honest result — the settlement batch itself remained completely untouched (its own `amount`/`status` never rewritten), while the order's own outstanding correctly became **negative** (−1,700,000 in the live QA session), visible on both the admin party-detail page and the professional's own Revenue tab, exactly as `test_a_refund_after_full_settlement_produces_a_negative_outstanding` already proved in isolation. No clawback was invented or automated — a real business decision, left open, per the task's own explicit instruction.

**Settlement model (task §20/§21), the explicit Phase 1 decision:** an admin selects one or more specific outstanding **orders** for a party; each is settled in full, for an amount the system computes from the live ledger — never a free-typed admin figure that could drift from the real financial facts. `wp_bc_settlement_items` itemizes by order id, so "why is this party's outstanding balance X" (task §31) is always traceable to specific orders. Reversal is supported (a real, audited, non-destructive status flip, never a deleted row) and re-frees the covered orders for a future settlement — verified by both a unit test and, indirectly, by the live QA session's own re-settle-after-reversal logic path.

**Party/ownership model (task §9/§33):** a booking's `provider_id` is a `bc_professional` or `bc_business` CPT post id (never a WP user id, matching `ProviderLookup`'s own established convention) — the post TYPE determines `party_type`. `MyFinanceController` (the professional/business self-service endpoint) resolves identity **exclusively** from `ProviderLookup::for_user()` on the authenticated session — no client-supplied provider/business id parameter exists anywhere in this controller to attack. A deliberate, documented deviation from `MyAnalyticsController`'s own otherwise-identical shape: this controller does **not** fall back to `StaffService` — financial data defaults to owner-only visibility, per this document's own standing constraint from Step 17's planning pass, not silently widened. Adversarially tested: `test_professional_a_cannot_see_professional_b_financial_data` (including a forged `provider_id` request parameter, proven to have zero effect since none is ever read).

**Admin UI (task §24):** reuses `AdminShell` exactly like every other BeauClick admin page — an overview (platform totals, commission-rate configuration, outstanding-by-party list, recent settlements) and a per-party detail screen (outstanding orders with checkboxes, settle form, settlement history with reverse). No REST surface for admin actions, matching Loyalty/Campaign's own classic-`admin-post.php` convention for internal, low-frequency, capability-gated tooling. Every settlement create/reverse and every commission-rate change produces a real `AuditLogger` entry (`financial_settlement_created`, `financial_settlement_reversed`, `financial_commission_rate_changed`) — live-verified in the real admin audit log with the correct actor and Jalali timestamp.

**Professional/business UI:** the `درآمد` (Revenue) tab — a `ready: false` placeholder since V1, the single most-anticipated "coming later" promise this dashboard has carried across every prior planning document — is now real, reading `/financial/my-summary`. Deliberately shows only booking-derived receivable/settlement figures, never a Shop/B2B "gross order value" (that concept already exists, honestly labelled, on the Analytics tab — this step never duplicates or reinterprets it).

**Tests:** 36 new tests (`LedgerServiceTest`, `SettlementServiceTest`, `PaymentRefundIntegrationTest`, `MyFinanceControllerTest`, `FinancialAdminPageTest`) covering commission/receivable split and rounding correctness, idempotency (retried payment/refund hooks, duplicate settlement attempts), the full real cross-plugin order lifecycle (payment → ledger, cancellation → real refund → ledger reversal, Shop-order exclusion, Campaign+Membership stacking, reschedule non-interaction), settlement integrity (ownership checks, reversal, refund-before/after-settlement), adversarial cross-party isolation, and admin audit logging. Full backend suite: **805/805** (769 baseline + 36 new), zero regressions. Frontend: TypeScript clean, ESLint clean, Vitest **38/38** (unchanged), production build succeeds and deploys.

**Live QA:** performed against the real local WordPress site using the same QA admin account established in Step 17, plus a temporary, clearly-QA-labeled password set on an existing **demo** professional account (`bc_demo_sara_ahmadi` — fictional seed data, not a real person's credentials) so the real professional-facing Revenue tab could be verified end to end, not just via automated tests. Verified live and directly observed: a real booking payment recorded the exact expected commission (300,000) and receivable (1,700,000) split on a real 2,000,000-Toman order (itself already Campaign-discounted, confirming the "real, already-discounted total" basis in a live, not just tested, scenario); the admin Financial overview and party-detail pages showing the correct real figures; a real settlement created through the admin UI (outstanding → 0, audit-logged); a real cancellation of the now-settled, paid booking correctly issuing a genuine WooCommerce refund and reversing the ledger, with the resulting negative outstanding correctly visible on both the admin page and the professional's own Revenue tab. **Known limitation, disclosed rather than fabricated:** the browser automation pane became unresponsive again near the end of this session (the same category of tooling issue Step 17's own live QA already disclosed) before a dedicated mobile-viewport pass on the *admin* Financial page specifically could be captured (the Revenue tab's own mobile/RTL check — no horizontal overflow at 375px — was captured before this happened), and before a live-browser (as opposed to automated-test) cross-professional adversarial check could be attempted. The admin page reuses the same `overflow-x:auto` table pattern already mobile-verified elsewhere, and cross-party isolation is adversarially covered by `MyFinanceControllerTest` — but neither is claimed as live-browser-verified here.

**What this step deliberately did not build, and why:** any automated payout/disbursement (bank API, gateway payout, scheduled transfer) — explicitly out of scope per the task's own repeated instruction, and, per this document's own Financial Truth table, not realistically buildable before a manual settlement workflow has been exercised in production at least once; a real payment gateway; any change to `LoyaltyLedger` or a second loyalty balance (loyalty/referral points remain entirely separate from this ledger, confirmed by inspection — they were never touched); full double-entry accounting (§8's own explicit decision); any B2B/Shop financial settlement (out of this ledger's confirmed scope); a CSV export or general-ledger browser (task §24's own "not a full accounting ERP" instruction).

**No other item in this register or plan changed status as a result of this step.** Step 19 and Step 20 were not started. No `v2.3.0` tag was created. The rest of this document's V2.1/V2.2/V2.3 completion notes above remain the accurate historical record and are preserved exactly as written.

---

## V2.3 Step 19 — AI for Professionals & Businesses (Read-Only Insights) Implementation Notes

**Step 19 is complete.** A professional/business AI insight surface now exists inside the professional dashboard, reading only the caller's own, already-ownership-scoped analytics/financial/campaign data — strictly read-only narration, never an autonomous action, exactly as this document's own Step 19 definition required.

**AI-06 (the schema blocker) closed exactly as this document's own recommendation specified:** a new, wholly separate `wp_bc_ai_professional_conversations`/`wp_bc_ai_professional_messages` table pair, not a modification of the existing, live `wp_bc_ai_conversations`' `UNIQUE(user_id)` constraint. One deliberate refinement beyond the original sketch: the new table's identity column is `provider_id` (the owned `bc_professional`/`bc_business` CPT post id), not `user_id` — professional AI belongs to the *business*, the same identity every other Step 17/18-era table (`wp_bc_ledger_entries`, `wp_bc_campaigns`) already keys on, not the WP user. `user_id` is still stored (informational — who is currently allowed to use it) but is not the unique key. This also means the new table carries no `ai_context` column at all: unlike the customer-mode conversation (which accumulates discovery signals like `cityId`/`budget` across turns), a professional's question is always answered from **freshly rebuilt** real data on every single turn — "what's my revenue right now" must never read a stale, turn-one snapshot. Verified: the existing `wp_bc_ai_conversations`/`AssistantService`/`AssistantController`/customer AI panel are untouched — the full pre-existing `AssistantServiceTest`/`AssistantControllerTest` suites pass unmodified.

**Provider/route separation, per this document's own decision:** a wholly new `/ai/professional/messages` REST route (`ProfessionalAssistantController`), not a `context` parameter grafted onto the existing customer `/ai/messages` endpoint. A new `ProfessionalAssistantService` orchestrates guard → rate-limit → context-assembly → provider → persist, mirroring `AssistantService`'s own shape but deliberately simpler: no recommendation validation (this mode never recommends catalog entities — see Context scope below), no accumulated context merge.

**Provider abstraction, reused not duplicated (task §20):** the existing `ProviderInterface`/`AssistantResponse` contract is reused as-is. A new `ProfessionalProviderFactory` reads the identical `BC_AI_PROVIDER`/`BC_AI_API_KEY`/`BC_AI_MODEL` env configuration as the customer-mode `ProviderFactory` — one provider configuration serves both surfaces. The concrete classes differ (`ProfessionalAnthropicProvider`, `ProfessionalRuleBasedProvider`) because `AnthropicProvider`'s existing system prompt is hardcoded to a customer-discovery persona (`CatalogContext`, provider/service/product recommendation JSON) — architecturally wrong for business-data narration, and repurposing it in place would have required changing its public contract, risking the already-tested customer path for no benefit. Duplicating ~40 lines of stable `wp_remote_post` plumbing in a new, separate class keeps the existing surface at zero regression risk. `ProfessionalRuleBasedProvider` (the real fallback in this environment — no live Anthropic key configured, same Iranian-IP-access caveat already disclosed for the customer-mode adapter) never interprets the professional's free text as intent to look anything up; it only performs a deterministic Persian-keyword topic match against an **already-fully-resolved** `$context`, narrating real numbers already present in it, never inventing or re-deriving one. This also makes prompt injection structurally moot for the default/dev provider: there is no LLM to manipulate, only a keyword lookup into pre-computed real data.

**Context scope — deliberately narrower than the task's own candidate list (task §11/§17/§18/§19), a documented engineering decision:** `ProfessionalContext` assembles exactly three sources, all already-authoritative, already-ownership-scoped services this codebase already had: `MetricsService::for_provider()` (booking funnel, review count/avgRating, customer total/repeat/newInRange, top-5 service performance — Step 11/16's own analytics, no second calculation engine), `SettlementService::party_summary()`/`outstanding_orders_for_party()` (Step 18's own real ledger, owner-only per its own established boundary), and `CampaignService::all()` filtered to this provider's own or platform-wide **active** campaigns (Step 17's own real usage/discount facts, no independent eligibility or discount math). **CRM notes and raw review text were deliberately excluded from Phase 1**, per the task's own explicit "if too sensitive, exclude — don't include just because the database makes it accessible" instruction (§18/§19): CRM notes are free-text, professional-authored, potentially containing customer PII, with no existing safe-summarization code anywhere in this codebase (confirmed by this step's own architecture audit) — and, critically, every Phase 1 question this feature promises to answer ("how are my bookings performing," "what's my top service," "what's my revenue," "what campaigns are active") is already fully answerable from the three aggregate sources above, with zero raw customer identity ever reaching the AI provider. `beauclick-ai`'s plugin header gained real `Requires Plugins` entries (`beauclick-marketplace`, `beauclick-analytics`, `beauclick-financial`, `beauclick-campaigns`) — a hard, fail-closed dependency (matching `beauclick-privacy`'s own precedent of a plugin requiring many others for one cross-cutting concern) rather than the softer `class_exists()` optionality used elsewhere in this codebase, because `ProviderLookup` (the actual ownership/security boundary) must never be silently absent.

**Security (task §7/§8, the single most important requirement, restated because both this document and the task named it as the crux):** identity is resolved exclusively via `ProviderLookup::for_user()` against the authenticated session — no route, parameter, or stored field anywhere in this feature ever accepts a client-supplied `provider_id`/`professional_id`/`business_id`/`user_id`. `ProfessionalContext` never resolves ownership itself; every method requires an already-resolved `$provider_id` from the controller. The new conversation table's `provider_id` is looked up by the resolved id, never trusted from a request. Adversarially tested, not just happy-path: a full REST-layer sweep (`ProfessionalAssistantControllerTest`) proving a forged `provider_id`/`providerId`/`user_id` request parameter has zero effect on which provider's conversation is written to or which data is narrated; a full cross-professional data-leakage sweep (`ProfessionalContextTest`, `ProfessionalAssistantControllerTest`) seeding two professionals with distinguishable real booking/financial/campaign data and proving neither's AI response, conversation, or context ever contains the other's figures; prompt-injection payloads (including ones explicitly attempting to name another provider's id) rejected by the existing, reused `MessageGuard` before ever reaching a provider. **Live-verified, not only unit-tested:** two real QA professional accounts, each with genuinely different real booking/review/financial history, each asked the identical "وضعیت مالی من چیه؟" question through the real dashboard UI — each received a completely distinct, correct, real answer (one showing a real 1,700,000-Toman settled figure from Step 18's own earlier live QA data, the other showing real zeros for a professional with no financial activity), with no trace of cross-contamination.

**Staff and business scope (task §9/§10):** staff are deliberately excluded from professional AI in this phase — `ProfessionalAssistantController` resolves ownership via `ProviderLookup::for_user()` only, the identical deviation `MyFinanceController` already established for financial data (owner-only, no `StaffService` fallback), extended here on the same reasoning: business-data AI is at least as sensitive as the financial data it partly narrates. Adversarially verified: a real active staff membership (no owned CPT post) does not unlock the endpoint — a real 404, the same honest "no profile" response an ordinary customer gets. Business support is handled through the existing `ProviderLookup`/party-type-resolution architecture already proven by `MyFinanceController` — no new business identity system was built, and (confirmed by direct query, matching this document's own prior finding) no real `bc_business` CPT post exists in this dev database at all, so business-side live QA could not be performed; the code path is identical to the professional path and covered by a dedicated test (`test_business_post_type_resolves_the_business_party_type`) using a synthetic `bc_business` post, disclosed here as the honest boundary of what was actually exercised live.

**New capability:** `bc_use_professional_ai`, added to `RoleManager::professional_capabilities()` (and therefore, via the existing `array_merge`, to `business_capabilities()` too) — its own capability, not a reuse of `bc_use_ai_assistant` (which every customer, including a professional in their own shopper capacity, already holds), since the two surfaces read entirely different, differently-scoped data and should stay independently grantable/revocable. `CAPS_VERSION` bumped, matching this file's own established convention.

**Professional dashboard UI (task §25/§26):** a new "دستیار هوشمند" tab, slotted immediately after "درآمد" (Revenue) — the two most natural "ask a follow-up question about what I just saw" tabs sit adjacent, matching `dashboard-professional.tsx`'s own existing mechanical pattern for adding a ready tab (`NAV_ITEMS`, `READY_TABS`, one render line). Deliberately an in-tab panel (`ProfessionalAiTab.tsx`), not a `Modal`/drawer like the customer-facing `AiPanel` — a professional working through dashboard data wants the assistant alongside the rest of the dashboard, not as an interrupting overlay. Reuses the existing chat-bubble/quick-suggestion visual language (`bc-chat-bubble`, `Chip`, `LoadingDots`, `Input`/`Button` from the design system) rather than inventing a new one. The panel's own header text states plainly, in Persian, that the assistant is informational/read-only, reads only the professional's own data, may be AI-generated, and that some information may be unavailable — matching the task's own explicit "do not claim it knows everything" wording requirement.

**Rate limiting (task §28):** reuses the exact same transient-based mechanism and limit (15 messages/60 seconds), under its own separate key namespace (`bc_ai_professional_rate_{user_id}`) so a professional's customer-mode AI usage and professional-mode AI usage never share or exhaust the same bucket — verified by a dedicated test.

**Tests:** 4 new test files (`ProfessionalContextTest`, `ProfessionalRuleBasedProviderTest`, `ProfessionalAssistantServiceTest`, `ProfessionalAssistantControllerTest`) covering context aggregation correctness and cross-tenant isolation at the data layer, deterministic never-fabricate narration (including a payload deliberately containing an "ignore previous instructions" phrase, confirmed structurally inert against this provider), conversation reuse/isolation and independent rate-limiting at the service layer, and the full REST-layer adversarial sweep described above (capability gating, forged-parameter resistance, staff exclusion, admin-without-a-profile honesty, no client-facing ownership-mutation route). Full backend suite: **836/836** (805 baseline + 31 new), zero regressions — the 88-test `beauclick-ai` suite alone covers both the pre-existing customer-AI tests, unmodified, and this step's 31 new professional-AI tests. `beauclick-core/tests/bootstrap.php`'s plugin activation order was updated — `beauclick-ai` now activates after `beauclick-marketplace`/`beauclick-analytics`/`beauclick-financial`/`beauclick-campaigns` (its new real dependencies) and still before `beauclick-privacy` (which depends on it). Frontend: TypeScript clean, ESLint clean, Vitest **38/38** (unchanged — no new frontend unit test added for `ProfessionalAiTab.tsx`, matching `RevenueTab.tsx`'s own precedent of relying on live QA rather than a component test for a simple fetch-and-render tab), production build succeeds and deploys.

**Live QA:** performed against the real local WordPress site, using two distinct, pre-existing local QA professional accounts (`bc_demo_sara_ahmadi`, `bc_qa_test_pro` — both fictional/synthetic seed or QA data, neither a real person, per this session's own separate, explicit local-QA-credential-preparation task). Verified live and directly observed: the "دستیار هوشمند" tab renders correctly in the real professional dashboard nav; clicking a quick-suggestion chip and asking free-text questions both produce real, correctly-scoped answers narrating each account's own genuinely different booking/review/financial figures (including a correct real negative outstanding balance carried over from Step 18's own earlier live QA session, narrated honestly rather than hidden); switching to the second QA account produces a completely fresh, empty conversation and entirely different, correct real numbers, with zero trace of the first account's data — the strongest available confirmation, beyond the automated adversarial tests, that cross-professional isolation holds in the real running application, not only in test doubles.

**What this step deliberately did not build, and why:** any autonomous action (creating/cancelling/rescheduling a booking, modifying a price/campaign, creating a settlement, editing a CRM note, sending a customer message, changing verification/ownership) — the task's own explicit, repeated read-only boundary, enforced structurally by never exposing an action endpoint through this feature at all, not merely by prompt instruction; CRM notes or raw review text in the AI context (see Context scope above); staff access to professional AI (task §10's own explicit "safe default is owner-only" instruction, with no evidence found to justify widening it); a second, independent AI provider system (the existing `ProviderInterface` is reused, only its concrete implementations are new); any change to the existing customer-mode AI conversation model, table, or capability; cost/usage visibility (still correctly deferred, unchanged, until a real paid provider is actually configured, per this document's own prior V2.3 planning).

**No other item in this register or plan changed status as a result of this step.** Step 20 was not started. No `v2.3.0` tag was created. The rest of this document's V2.1/V2.2/V2.3 completion notes above remain the accurate historical record and are preserved exactly as written.

---

## V2.3 Step 20 — Growth & Professional Platform Quick Wins Implementation Notes

**Step 20 is complete.** All four independently-scoped items are real, tested, and live-verified against the running local site — no new backend systems, no new tables beyond one narrow column addition, every item reusing infrastructure that already existed.

**Pre-implementation audit (per this step's own instruction not to trust prior completion claims):** re-verified Step 19 (AI for Professionals) by independent code inspection before starting — confirmed genuinely real (route, ownership resolution via `ProviderLookup::for_user()`, React tab wiring, 4 real adversarial test files, commit `97b3686` matches the claim exactly). Re-confirmed the local commit `35746b3` ("fix(ui): align wordpress branding and header search") is legitimate, unrelated UI/branding work, was preserved untouched, and is now included in this step's own commit history alongside the new work.

**Item 1 — B2B quote request/accept UI.** The backend (`B2BController`'s `/b2b/quotes*` routes, `QuoteService`) was confirmed complete and untouched-by-necessity since V2.1; there was previously no UI anywhere — not a customer one, and not even an admin one for the counter-quote/pricing step.
- **Customer side:** a real B2B buyer (an approved `bc_business_accounts` row) has no marketplace listing and therefore cannot reach the React dashboard shell at all (confirmed by audit — `dashboard-professional.tsx` is the only dashboard mount, and it's gated on a `bc_professional`/`bc_business` CPT post, which a pure wholesale buyer doesn't have). The quote UI was therefore added to the one page every B2B buyer actually reaches: `page-b2b.php` (server-rendered PHP + vanilla JS, matching this page's own existing `bc-b2b-apply-form` convention exactly — `fetch()` + `X-WP-Nonce`, no build step). Each wholesale catalog card gained a quantity input + "افزودن به درخواست پیش‌فاکتور" button; selections accumulate client-side into a quote-request builder card; submitting `POST`s to `/b2b/quotes`. A new "پیش‌فاکتورهای من" section (PHP-rendered from `QuoteService::for_business_account()`, using the existing shared `JalaliDate::format()` for dates) lists every quote with its real status, resolves each line item's product name live via `wc_get_product()`, and — once an admin has priced it — shows a real "تأیید و پرداخت" button that `POST`s to `/b2b/quotes/{id}/accept` and redirects to the real WooCommerce payment URL the endpoint already returns.
- **Admin side:** a new `QuotesAdminPage` (`beauclick-b2b/src/Admin/QuotesAdminPage.php`), reusing `AdminShell` and the exact classic-`admin-post.php` convention `AccountsAdminPage` already established for this plugin (not a REST-driven admin UI, matching this plugin's own precedent for low-frequency internal tooling). Lists every quote grouped by business, with an editable price-per-line form for `requested` quotes that calls `QuoteService::submit_quote_prices()` directly. Every successful pricing action writes a real `AuditLogger` entry (`b2b_quote_priced`, entity `quote`, previous/new `status`) — a genuinely new audit-log write path, not merely a fix to an existing one (see Item 4).
- **Live-verified, not only unit-tested:** logged in as `bc_qa_business`, added two real products to a quote request, submitted it (`در انتظار قیمت‌گذاری` shown correctly); logged in as `bc_qa_admin`, priced both lines through the real admin form (`623,000` تومان total computed correctly, `b2b_quote_priced` appeared correctly at the top of the real admin audit log with the right actor); logged back in as `bc_qa_business`, saw the quote correctly flip to `آماده تأیید` with the real quoted total, clicked "تأیید و پرداخت", and landed on a real WooCommerce payment page showing the exact quoted line-item prices and the exact quoted total (`623,000` تومان) — a direct, live confirmation of "displayed price equals charged price" for a real order, not just an assertion in a test double.

**Item 2 — Basic marketplace text search (MKT-02).** Confirmed by audit that `MarketplaceController::browse()` had no `q`/free-text parameter at all (not merely "no fuzzy matching," as one gap-register row's own imprecise wording suggested) — the platform's only "search" was structured filters (city/district/specialty/price/rating). CRM's existing digit-normalization idea (`CrmService::normalize_digits()`) was the pattern to reuse, not a literal SQL `LIKE` precedent — CRM's own actual implementation is a PHP `str_contains()` filter over an already-small, pre-fetched result set, which doesn't fit `browse()`'s SQL-paginated scale, so the *normalize-then-substring-match* idea was reused while the *filtering strategy* was adapted to SQL.
- **Schema:** `AddProviderSearchTextColumn` migration adds one `search_text TEXT` column to `wp_bc_provider_index` (same `dbDelta`-diffs-the-full-CREATE-TABLE convention as the existing `AddProviderRankingSignalsColumn`), and — since adding a column doesn't populate it for providers indexed before the migration runs — the migration also backfills every existing row via `Indexer::sync()` itself (not a second name+bio computation), so the feature is immediately correct on any environment the migration runs on, not only for providers someone happens to re-save later.
- **Indexing:** `Indexer::sync()` now also computes `search_text` = `TextNormalizer::normalize(name + stripped bio)`, bounded to 2000 characters (bio is free text with no practical size cap; LIKE performance/storage shouldn't scale with how much a provider chose to write). `TextNormalizer` (`beauclick-marketplace/src/Search/TextNormalizer.php`) duplicates `CrmService::normalize_digits()`'s five-line Persian/Arabic-digit-to-ASCII logic rather than reaching across the plugin boundary for it — the same "keep the existing surface at zero regression risk" call this codebase already made for `ProfessionalRuleBasedProvider` (Step 19).
- **Query:** `browse()` gained a `q` param — `WHERE search_text LIKE %s` (both sides normalized identically, `$wpdb->esc_like()`-escaped), composing with every existing structured filter via `AND`, never `OR` (live-verified: a name match in the wrong city is correctly excluded). `search_performed`'s existing analytics event gained one new boolean field, `textSearch` — still never the raw query text itself, matching this codebase's own "no raw sensitive text in analytics" standard.
- **SSR + header search wiring:** `bc_get_providers()` (theme helper) gained the identical `q` support, reusing `TextNormalizer` via the same `class_exists()` cross-boundary-reference precedent `RankingPresenter::ORDER_BY` already established for this exact helper. `page-marketplace.php` gained a real search input (GET form, preserves `city_id`/`specialty_id`), a "نتایج جستجو برای «…»" heading, and a clear-search link. `HeaderSearch.tsx` — the header search overlay a prior UI pass (`35746b3`) had deliberately left as a specialty/city-only client-side filter, with its own docblock explicitly naming free-text search as this exact future step — now falls through to a real `/marketplace/?q=` navigation whenever the typed text matches no specialty/city chip, replacing what was previously a dead end (`onSubmit` doing nothing) with the real search this step built.
- **Live-verified:** `q=سارا` correctly returned exactly the one matching provider by name; `q=کاشت` correctly matched a provider only via bio text (no name match); a Persian-digit bio (`۲۰ سال تجربه`) correctly matched an ASCII-digit query (`20 سال`), proving the digit normalization works end to end; the header search overlay's fallback button correctly navigated to a real `q=` URL. **Known, expected limitation (not a bug):** a plain substring match doesn't match reordered words — a bio containing "ناخن و کاشت" does not match the query "کاشت ناخن" (word order reversed) — this is the literal, intentional boundary of "plain LIKE-based," not fuzzy/typo-tolerant matching, exactly as this step's own scope (and the still-deferred MKT-02 fuzzy-search recommendation) describes.

**Item 3 — Professional/business notification-preferences UI (NOTIF-07).** Confirmed by audit that `PreferenceService`/`NotificationsController` are already fully generic — every route resolves the owner via `get_current_user_id()` alone, with no customer-specific concept anywhere in the backend, and a professional/business account is a real WP user (per `RoleManager`'s own capability-superset design) — so this item required zero backend changes. `NotificationsTab.tsx` (`beauclick-marketplace`'s dashboard equivalent, under `app/src/features/dashboard/professional/`) composes the exact same `NotificationPreferences`/`NotificationsList` components the customer `AccountTab.tsx` already uses — literal reuse, not a reimplementation, so the two surfaces can never silently drift on what a toggle does. Wired into `dashboard-professional.tsx`'s existing `NAV_ITEMS`/`READY_TABS` mechanical pattern (a new "اعلان‌ها" tab, after "پیام‌ها"). No new frontend unit test added, matching this file's own established precedent (`RevenueTab.tsx`, `ProfessionalAiTab.tsx`) of relying on live QA rather than a component test for a simple fetch-and-render composition tab.
- **Live-verified:** logged in as `bc_demo_sara_ahmadi`, the new "اعلان‌ها" tab renders all five real preference toggles (all defaulting to "فعال") and an honest "اعلانی برای شما ثبت نشده است" empty state; toggling "یادآوری بازگشت" off produced a real `PATCH /notifications/preferences` (200 OK) and was confirmed persisted directly in `wp_bc_notification_preferences` (`enabled = 0` for that professional's own `user_id`) — the same generic backend, genuinely reused, not merely claimed to be.

**Item 4 — Admin audit-log REST gap fix (ADMIN-05).** Confirmed by audit: `B2BController::approve_account()`/`reject_account()` (REST, admin-permission-gated) reach the exact same action as `AccountsAdminPage`'s wp-admin form, but never called `AuditLogger` — a real, reachable, unaudited path. Fixed by adding the same `AuditLogger::record()` call (`b2b_account_approved`/`_rejected`, entity `business_account`, previous/new `approval_status`) directly in the REST controller — **deliberately not** by delegating to `AccountsAdminPage::reject_and_log()`, because that method hardcodes a wp-admin-only default rejection reason with no reason field of its own, while the REST route already accepts a real caller-supplied `reason` that must be the one actually logged, not silently overwritten.
- **A second candidate of the same class, found and deliberately not fixed here, per this task's own bug discipline (§24):** `beauclick-loyalty\LoyaltyController`'s REST admin routes (tier/plan/benefit CRUD, membership grant/cancel — all `bc_manage_platform`-gated, all currently unused by any frontend) call their services directly with no `AuditLogger` call, while `LoyaltyAdminPage`'s wp-admin handlers for the identical actions do log. This is a real, same-shape gap — not fixed as part of Step 20 because it's unrelated to any of this step's four named items and not essential to any of them functioning; flagged here explicitly, matching this document's own V2.3 Risks section, which already disclaimed that its audit "found exactly one [gap], but did not claim to have found every possible instance."
- **Tested:** `B2BControllerTest.php` (new) proves the REST approve/reject path now writes the identical audit-log shape `AccountsAdminPageTest` already proved for the wp-admin path, including that the REST caller's own `reason` (not a hardcoded default) is what gets logged. Live-verified indirectly: the real admin audit log correctly showed `b2b_quote_priced` (Item 1's new admin action) at the top of the feed during this session's own live QA, confirming `AuditLogger` writes correctly reach the real admin UI end to end.

**Tests:** 15 new backend tests (`B2BControllerTest` ×2, `QuotesAdminPageTest` ×2, `MarketplaceControllerTest` ×5 new `q`-search cases, `IndexerTest` ×1 new `search_text` case — plus the pre-existing suites' own coverage untouched). Full backend suite: **846/846** (831 baseline + 15 new), zero regressions. Frontend: 2 new/updated `HeaderSearch.test.tsx` cases (the no-chip-match fallback now asserts a real `q=` navigation instead of a dead end) — full Vitest suite **48/48**, TypeScript clean, ESLint clean, production build succeeds and deploys to the theme (`npm run build`'s `postbuild` step).

**PHP lint, precisely stated:** `php -l` (syntax) is clean on every new/modified file. Full `WordPress-Extra` PHPCS conformance is **not** claimed — an audit during this step confirmed the project's own already-shipped code (e.g. `QuoteService.php`, untouched by this step) has pre-existing PHPCS findings of the same classes newly-written code in this step also has (short array syntax, short ternaries, `PrefixAllGlobals` on page-template-scope variables) — this appears to be, and has apparently always been, a `php -l`-only bar for this codebase's page templates, not a full-WPCS one. New code was written to match the surrounding file's own existing style exactly, introducing no new violation category beyond what the codebase already tolerates everywhere.

**Live QA:** performed against the real local WordPress site (PHP's built-in server, `php -S`, routed through a dev-only router script for pretty permalinks — not part of the application, not committed), using the pre-existing QA accounts (`bc_qa_business`, `bc_qa_admin`, `bc_demo_sara_ahmadi` — a temporary, clearly-QA-labeled password was set on each via `wp_set_password()`, matching Step 18's own established precedent; the site owner's own `admin` account was never touched). All four items verified live and directly observed, detailed above. **Known limitation, disclosed rather than fabricated:** a handful of `403 Forbidden` console errors were observed during this session, but could not be tied to any specific failing request in the network log (every logged request returned `200 OK`) — most plausibly a stale WordPress admin-bar heartbeat nonce left over from switching between three different QA user sessions inside the same browser tab during this QA pass, not a request any of this step's own features make. Not confirmed as a real defect; noted honestly rather than either ignored or overstated.

**What this step deliberately did not build, and why:** fuzzy/typo-tolerant search (explicitly out of Item 2's scope, still evidence-gated per MKT-02's own recommendation); a dedicated business-only dashboard shell (the pure-wholesale-buyer gap this audit re-confirmed — Item 1's UI was placed on the one page such a buyer can already reach, `page-b2b.php`, rather than building a new dashboard mount neither this task nor any prior gap-register row asked for); new professional-specific notification categories (e.g. "new booking received," "new review") — Item 3 reuses the existing five customer-framed categories verbatim, exactly as the task's own "mirroring AccountTab.tsx exactly" wording specifies; new categories would be a real, separate, undocumented scope expansion; a fix for the `beauclick-loyalty` REST audit-log gap found during Item 4's own audit (see above — flagged, not fixed, per bug discipline).

**No other item in this register or plan changed status as a result of this step.** Steps 17, 18, and 19 remain unchanged (re-verified, not re-built). No `v2.3.0` tag was created.

---

## V2.3 Final Release Audit

**Audit date:** 2026-08-15. **Baseline audited:** `c6c6e5f` (`master`), the Step 20 completion commit — verified as `HEAD`, matching `origin/master` exactly at both the start and end of this audit, clean working tree throughout except for this audit's own fix commit. All five historical tags (`v1.0.0`→`8494c7b`, `v1.0.1`→`a8117ec`, `v2.0.0`→`67325b5`, `v2.1.0`→`d144509`, `v2.2.0`→`9c980ab`) re-verified byte-for-byte unchanged. `v2.3.0` confirmed not to exist.

**Method:** five independent, parallel, evidence-cited audits — one per Step (17–20), plus a dedicated global cross-cutting pass covering every V2.3 REST route's authorization and every V2.3 migration's safety/idempotency/indexing — each required to cite real `file:line` evidence, none permitted to trust a prior completion note's own claim. Followed by full automated test execution (backend, frontend, TypeScript, ESLint, `php -l`, phpcs) and reconciliation against `PRODUCT_GAP_REGISTER.md`.

**Result — two release-blocking defects found and fixed, one high-severity defect found and fixed:**

1. **BLOCKER — `beauclick-loyalty\LoyaltyController`'s REST admin routes wrote no audit-log entry.** This was a known, previously-disclosed gap (see the V2.3 Step 20 notes above, which found and explicitly deferred it as "unrelated to any of this step's four named items"). Tier/plan/benefit CRUD and membership grant/cancel — all real, `bc_manage_platform`-gated, money/trust-adjacent actions reachable via REST — bypassed the audit trail their wp-admin twin (`LoyaltyAdminPage`) already writes to, the same bug class this release explicitly fixed twice elsewhere (B2B account approve/reject, B2B quote pricing). Given the release's own V2.3 Definition of Done explicitly requires "complete audit-log coverage across every reachable action path, not just the wp-admin one," this was escalated from a deferred note to a genuine release blocker. **Fixed:** every mutating method in `LoyaltyController` now calls `AuditLogger::record()` directly, mirroring `LoyaltyAdminPage`'s own action-type naming (`loyalty_tier_created/updated`, `loyalty_plan_created/updated`, `loyalty_benefit_created/updated/deleted`, `loyalty_membership_granted/cancelled`). 6 new regression tests added to `LoyaltyControllerTest.php`.

2. **BLOCKER — Step 19's professional-AI conversation tables were never wired into `beauclick-privacy`'s export/deletion sweep.** `wp_bc_ai_professional_conversations`/`_messages` (new in this release) had no `export_for_user()`/`forget_user()` method and no caller anywhere in `beauclick-privacy`. A professional's business-AI chat history was therefore neither included in their own data export nor erased when their account-deletion request was approved — an orphaned data domain the deletion sweep structurally missed, breaking the same self-service export/deletion guarantee V2.2 Step 14 established and the V2.2 Final Release Audit's own `AccountEraser` fix reinforced. **Fixed:** `ProfessionalAssistantService` gained `find_conversation_for_user()`/`export_for_user()`/`forget_user()`, mirroring the customer-mode `AssistantService`'s own methods exactly; both are now called from `ExportService::collect()` and `DeletionService::process()` alongside the existing customer-mode calls. 4 new regression tests in `ProfessionalAssistantServiceTest.php`, 1 new cross-domain regression test in `DeletionServiceTest.php`, `ExportServiceTest.php`'s expected-files assertion extended.

3. **HIGH — `B2BController::submit_quote_prices()` (REST) still wrote no audit-log entry after Step 20.** Step 20's Item 4 fixed this exact bug class for `approve_account()`/`reject_account()` but missed the quote-pricing route — confirmed by both the Step-20-specific audit and the global cross-cutting audit independently. `QuotesAdminPage::price_and_log()` (also added in Step 20) already logs `b2b_quote_priced` for the wp-admin path; the REST path — the only path the new B2B quote-request UI's admin counter-quote flow actually reaches in practice — did not. **Fixed:** `submit_quote_prices()` now writes the identical `b2b_quote_priced` audit entry `price_and_log()` does. 2 new regression tests in `B2BControllerTest.php`.

**Everything else audited clean, no other release blocker found.** Summary by area (full detail in each parallel audit's own findings, not duplicated here):

- **Step 17 (Campaign Engine):** admin-only authorship, 100% server-side eligibility/discount resolution, fixed Membership-then-Campaign fee order verified non-compounding by a real regression test (10%+15% on 1,000,000 → exactly 750,000), zero cart/coupon hook introduced (grep-confirmed across every `beauclick-*` plugin), reschedule/receipt/B2B non-interference all confirmed. One MEDIUM, non-blocking finding: `EligibilityResolver::is_eligible()`'s usage-count check and `CampaignService::record_usage()`'s insert are not atomic across different bookings — a narrow TOCTOU race that can let `usage_limit_total`/`usage_limit_per_customer` be overrun by a small margin under real concurrent load. Not fixed in this pass (requires a locking/atomic-UPDATE redesign, not a release-blocking severity) — logged as a follow-up.
- **Step 18 (Financial Ledger & Settlement):** the highest-scrutiny step, audited with zero findings above LOW severity. Ledger genuinely append-only (no UPDATE/DELETE anywhere), commission rate captured immutably per-entry at write time (not a live join to mutable config), idempotent via `UNIQUE(entry_type, reference_type, reference_id)` + `INSERT IGNORE`, refund reversal and cancellation-refund wiring both correct and adversarially tested, owner isolation verified against a forged-parameter attack, staff correctly excluded by default, no automated payout API exists. Negative-outstanding-after-refund-post-settlement is a real, explicitly-documented business decision (no auto-clawback), not a bug.
- **Step 19 (AI for Professionals):** core security boundary — server-side-only identity resolution, capability gating, per-party-scoped reads — genuinely implemented and backed by real adversarial tests (two distinct professional identities, real DB rows, real negative assertions against leaked data). The one BLOCKER (privacy sweep gap) is fixed above. Two MEDIUM, non-blocking spec deviations logged for awareness: `CrmService::is_customer_of()` is not actually called (the shipped context excludes individual CRM records entirely instead, which is safe but not what the plan describes); `validate_recommendations()`'s ownership check was never added because professional-mode AI never emits a `recommendations` array in this phase — will need real attention if a future phase lets professional AI reference a CRM-scoped entity.
- **Step 20 (Growth & Platform Quick Wins):** free-text search is genuinely plain-LIKE (not fuzzy, matching spec), parameterized (no SQL injection), digit-normalization functionally correct though duplicated rather than literally reused from CRM (MEDIUM, drift risk only). Header search confirmed genuinely wired end-to-end (not merely rendered) by real test assertions on `window.location.href`. B2B quote UI confirmed to read its displayed total from the same source `QuoteService::accept()` charges, never a separately-computed value. The audit-log gap (Item 4) is fixed above for the one instance Step 20 itself missed.
- **Global authorization audit:** every REST route across all V2.3-touched plugins declares a real `permission_callback` (`RestController::route()` makes a missing one a boot-time fatal, structurally). No IDOR found — every ownership-sensitive route resolves identity server-side (`ProviderLookup::for_user()`, `get_current_user_id()`), never from client input; the few routes that do accept a client-supplied id (`B2BController::accept_quote`, `StaffController::remove_staff`) all independently re-validate ownership before acting. One MEDIUM, non-blocking finding: `B2BController::set_tiers` (pricing-tier configuration, pre-existing since before V2.3) has no audit logging anywhere and no wp-admin twin — not fixed here (pre-existing, not a V2.3 regression, not money-adjacent in the same direct way as pricing/settlement actions) but logged as a follow-up alongside a structural recommendation: no shared mechanism ties a `bc_manage_platform`-gated REST mutation to mandatory audit logging, which is why this bug class has now recurred three times across two plugins — a base-class or registry-based enforcement is worth designing, not another one-off patch.
- **Database/migrations:** all four new V2.3 migrations (`CreateCampaignTables`, `CreateFinancialTables`, `CreateAiProfessionalTables`, `AddProviderSearchTextColumn`) confirmed additive-only, `dbDelta`-compatible, idempotent via the existing migration ledger, with unique constraints correctly enforcing every correctness-critical invariant (campaign double-application, ledger idempotency, `wp_bc_ai_professional_conversations`'s `UNIQUE(provider_id)` correctly avoiding the old `UNIQUE(user_id)` design mistake). Two LOW, non-blocking findings logged: no supporting index on `bc_ledger_entries.entry_type` for admin-only aggregate queries; `search_text LIKE '%term%'` necessarily full-scans (explicitly acknowledged/accepted tradeoff in the migration's own docblock, not an oversight). One MEDIUM, pre-existing (not V2.3) finding re-confirmed rather than newly discovered: `QuoteService::accept()` has no DB-level state guard against a concurrent double-accept race — logged, not fixed (pre-existing since before V2.3).
- **PHPCS:** the same repo-wide, pre-existing short-array-syntax/alignment style debt already documented in the V2.2 Final Release Audit was re-confirmed present at a comparable rate in an untouched V2.1 plugin (`beauclick-auth`) — not a V2.3 regression, not fixed here (style-only, out of this audit's release-blocker scope, matching the V2.2 precedent).

**Test suite results:** backend PHPUnit **857/857** (846 baseline + 11 new regression tests from this audit's own fixes — 2 from B2B, 6 from Loyalty, 5 across `ProfessionalAssistantServiceTest`/`DeletionServiceTest`), frontend Vitest **48/48** (unchanged), TypeScript clean, ESLint clean, `php -l` clean across every V2.3-touched plugin.

**Live QA — explicitly not performed in this audit pass.** This audit was conducted entirely through code-level inspection (five parallel evidence-cited agent audits reading real `file:line` source) and automated test execution (PHPUnit/Vitest/tsc/ESLint/`php -l`/phpcs) — no local WordPress dev server was started and no browser interaction with the running application occurred at any point. The QA accounts named in this audit's own brief (`bc_qa_customer`, `bc_demo_sara_ahmadi`, `bc_qa_test_pro`, `bc_qa_admin`, `bc_qa_business`) were **not exercised live**; every finding above traces to source-code inspection and adversarial *test-code* review (confirming the existing tests genuinely simulate cross-tenant attacks with real DB rows, not mocks), not to observed behavior of the running application. This is disclosed here explicitly rather than fabricating live-QA coverage that did not happen — a materially different (narrower) verification method than the V2.1/V2.2 Final Release Audits, which did perform live browser QA.

**Production configuration** (unchanged from the V2.2 Final Release Audit, re-confirmed not re-litigated): SMS gateway, SMTP, and the real Zarinpal payment gateway remain `EXTERNAL_CONFIGURATION`/`NOT_CONFIGURED`. Backup and error-monitoring integration remain `MISSING`. AI provider remains `NOT_CONFIGURED` (rule-based fallback genuinely `IMPLEMENTED`). Hosting/SSL `NOT_VERIFIED` (local dev only).

**V2.4 boundary confirmed:** exhaustive repo-wide search finds zero implementation of fuzzy search, portfolio upload, notification center, automated payout, Campaign Phase 2 (cart-based promotions), Realtime Communication, Native Mobile, or broader staff-permission granularity — only planning-doc language and the existing, already-documented evidence-gating reasoning for each.

**Final decision: `V2.3 READY FOR RELEASE`**, conditioned on the caveat above: this audit's confidence is based on code-level and automated-test verification only, not live browser QA. All three findings classified BLOCKER/HIGH above are fixed, tested, and the full suite re-passes with zero regressions; no unresolved release-blocking issue remains; historical tags are confirmed untouched; `v2.3.0` is confirmed not to exist. Per the audit's own explicit instruction, **no `v2.3.0` tag has been created** and the Global UI/UX & Product-Wide Audit has not been started — both remain separate, explicit next steps pending product-owner approval. Given the live-QA gap disclosed above, the product owner may reasonably want a live verification pass (or to run this audit's own Live QA section against a running environment) before tagging, even though no code-level blocker remains open.

**Update, post-tagging:** `v2.3.0` was subsequently tagged on this same commit. This section's own text above ("no `v2.3.0` tag has been created") reflects the moment it was written, not current state — left unedited rather than rewritten, per this document's own practice of not erasing historical findings; `docs/roadmap/PRODUCT_GAP_REGISTER.md`'s Version Ledger and the root `BeauClick_Version_2_Roadmap.md` are the sources of truth for current tag state.

---

## Global UI/UX & Product-Wide Audit

**Audit date:** 2026-08-18. **Baseline:** `v2.3.0` (`c505c20`), plus the one subsequent documentation-only commit (`52b986a`, V3 Phase 2 architecture discovery docs — no code). **Trigger:** a previously-shipped, now-fixed regression (the header search icon button had no click handler for a period during V2.3 UI work — see `35746b3 fix(ui): align wordpress branding and header search`) raised the question of what else might look wired but isn't. This audit exists specifically to answer that question product-wide, before `v2.3.1`.

**Method — real, not code-only.** Unlike the V2.3 Final Release Audit above (which explicitly disclosed it never started a server), this pass ran the actual local stack: PHP's built-in server (`wordpress/router.php`), the real MySQL dev database, the real seeded QA/demo accounts, and the built React app-shell — then drove it with a real browser (navigation, DOM inspection, network/console monitoring), not source-reading alone. Static analysis (`grep` across `app/src` and the theme for `href="#"`, empty-handler, and TODO/placeholder patterns) supplemented, not replaced, live verification.

**Scope actually covered, in depth:** full OTP login/resend/wrong-code/logout flow; the header search icon overlay and the homepage hero search bar, including a real free-text match against the marketplace backend; the complete 5-step booking flow from a real provider card through to a real WooCommerce order, payment, and order-received receipt, including the live campaign discount; the professional dashboard (Overview, Revenue/Financial Ledger, AI tabs) and the B2B wholesale catalog + quote-request history, both with real QA data; the **customer account area end-to-end** (see the dedicated section below — this was audited as a first-class surface, not a footnote); a static dead-interaction sweep of every React component under `app/src`.

**Scope sampled or not reached live this pass** (disclosed rather than silently skipped, per this document's own §27 discipline): the wp-admin-side BeauClick admin pages (`AuditLogPage`, `UsersAdminPage`, `OperationsHealthPage`, campaign/settlement admin) were reviewed at the source level only — `AdminShell.php`'s shared header/stat-card/empty-state/horizontally-scrollable-table wrapper (scoped only to `page_beauclick` admin screens, never touching native wp-admin) gives structural confidence, but no live click-through happened, because setting a temporary password on the `bc_qa_admin` QA account (needed since it has no phone/OTP path) was correctly refused by this session's own auto-mode safety classifier as a credential-modification action; a full B2B quote-acceptance round trip, the professional CRM/staff/calendar tabs in depth, and a systematic 6-breakpoint sweep of every page (only the account/dashboard surfaces got the full 375/390/412 pass) were not exhaustively covered given this pass's time budget. None of this is claimed as "audited clean" — it's undone, not passed.

### Findings

**BUG — fixed.** OTP resend leaves the stale code in the input field ([app/src/features/auth/AuthFlow.tsx](../../app/src/features/auth/AuthFlow.tsx)). `requestOtp()` is shared by both the initial "دریافت کد تأیید" submit and the "ارسال دوباره کد" resend button, but never cleared the `code` state — a user who requests a new code after one fails (wrong/expired) has the old code silently sitting in a `maxLength={6}` field, blocking correct re-entry and producing a confusing "کد تأیید نادرست است" on the *new* code. Reproduced live during this audit's own login testing before the fix existed. **Fix:** `setCode('')` added alongside the existing `setStep`/`setCooldown` calls on a successful `request-otp` response. Classification: BUG (not a design preference) — P2, since the workaround (manually clearing the field) is available but not obvious.

**NAVIGATION — fixed.** A logged-out visit to any WooCommerce `/my-account/*` endpoint rendered WooCommerce's own default username/password login form — the exact experience `beauclick-auth`'s phone/OTP flow was built to replace for every normal customer/professional/business account (AUTH-09 in the Gap Register: "passwords are no longer part of the normal customer/professional/business UX at all"). `page-dashboard.php` already redirects a logged-out `/dashboard/` visit to `/auth/`; `/my-account/*` had no equivalent, so a bookmark, shared link, or session that expired mid-visit landed a normal account on a form it structurally cannot use (it has no password). **Fix:** new [wordpress/wp-content/themes/beauclick/inc/account-redirect.php](../../wordpress/wp-content/themes/beauclick/inc/account-redirect.php), a `template_redirect` hook sending any logged-out `is_account_page()` visit to `/auth/`, mirroring the existing dashboard pattern; verified live not to affect `/checkout/` (guest checkout still reachable) and not to fire for logged-in visits. Classification: NAVIGATION/UX inconsistency — P1, since it's a genuine dead end for a normal account, not merely cosmetic.

**ACCESSIBILITY/CONSISTENCY (Persian/RTL) — fixed.** The AI assistant's rule-based narration (`beauclick-ai\Professional\ProfessionalRuleBasedProvider` and `beauclick-ai\RuleBasedProvider`) rendered numbers in raw Latin digits with English-style comma thousands separators (`"9 رزرو"`, `"1,700,000 تومان"`), while every other user-facing number in the product — dates, prices, receipts, dashboard stats, even the OTP SMS text itself — renders in Persian digits with the `٬` separator. Found live in the professional dashboard's "دستیار هوشمند" tab. Violates this document's own cross-cutting standard ("Persian-number-aware... no unnecessary English string where a Persian equivalent exists") and directly undercuts §22's "AI should feel like part of BeauClick, not a separate product bolted onto it." **Fix:** both providers now pass their narrated text through a small `strtr()`-based Persian-digit/separator conversion (mirroring `beauclick-auth\Otp\OtpService::to_persian_digits()`'s own established, minimal, locally-scoped pattern — no new shared cross-plugin utility introduced) at their single return point. Three existing PHPUnit tests that had encoded the Latin-digit output as "expected" were updated to assert the corrected Persian-digit output, not weakened. Classification: internationalization/consistency bug — P2.

**PRODUCT GAP (documented, not fixed).** A booking-flow order's receipt (`/checkout/order-pay/{id}/`, order-received state) shows `آدرس صورتحساب: نامعلوم` ("Unknown") for billing address, even when the customer has a real saved WooCommerce billing address — because `BookingOrderBridge` creates the order directly via `wc_create_order()`/`add_product()` (by design, to keep booking orders off the WooCommerce cart — see COM-05) without copying the customer's billing address onto the order. Functionally inconsequential (a service booking needs no delivery address), but reads as an incomplete/broken-looking receipt. Not fixed in this pass: populating it means touching `BookingOrderBridge`'s order-creation path, which is backend order-construction logic, not a pure UI change — left for a deliberate, reviewed change rather than folded into a UI-polish commit. Classification: PRODUCT GAP (cosmetic), P3.

**Confirmed clean, not just assumed:** the header search icon and its overlay (specialty/city chip filtering, free-text fallback to `/marketplace/?q=`) — the specific surface this audit exists because of — was re-verified end-to-end via real navigation and found correctly wired, no regression since `35746b3`. The 5-step booking flow, WooCommerce payment bridge, campaign discount application, and order-received receipt all produced correct real data with no console/network errors beyond expected 4xx from this audit's own deliberate wrong-OTP testing.

### Customer Account Dashboard Audit

**This section is the mandatory dedicated area required by the audit brief — actually tested, not summarized as "checked."**

1. **Actual route.** The customer's post-login landing surface is `/dashboard/` (`wordpress/wp-content/themes/beauclick/page-dashboard.php`), which mounts the `dashboard-customer` React bundle for any logged-in user without `bc_professional`/`bc_business`/staff status — **not** `/account/` or a bare `/my-account/`. `/my-account/*` is a second, WooCommerce-native surface, reached from the dashboard's own "حساب کاربری" tab.
2. **Login → dashboard flow.** Verified live end-to-end with `bc_qa_customer` (phone `09129998877`): `/auth/` → OTP → `wp_set_current_user()`/`wp_set_auth_cookie()` → redirect to `/dashboard/` → real booking history rendered immediately, Jalali-dated, correctly scoped to that one account.
3. **Overall Persian status: fully Persian.** Every label, tab name, button, success/error message, and status pill checked (dashboard nav, Account tab, WooCommerce My Account nav/labels/buttons, address form field labels including `کدپستی`/province dropdown with all 31 real Iranian provinces) rendered in Persian. No untranslated English or raw WordPress/WooCommerce default strings found anywhere in this surface.
4. **RTL status: correct.** `dir="rtl"`/`lang="fa"` forced at the document root (`functions.php`'s `language_attributes` filter, unconditional — not dependent on wp-admin's language setting); confirmed via computed style (`direction: rtl`) on both the dashboard and the WooCommerce account pages.
5. **Profile fields.** The BeauClick-native "حساب کاربری" tab (`app/src/features/dashboard/customer/AccountTab.tsx`) does **not** itself host name/address editing — it's honest about this, stating in Persian that account/address editing happens on WooCommerce's own "حساب کاربری" page, with a real link. Name, last name, display name, and email are editable there (`/my-account/edit-account/`); all fields present, Persian-labeled, prefilled with real values.
6. **Address management: fully functional, verified with a real round-trip.** `/my-account/edit-address/` shows the real saved billing address (province YZD/یزد, city, street, postcode, matching the design brief's Yazd-first, all-31-provinces location model). A safe field (`billing_address_2`, empty by default) was edited to `واحد ۳ (تست موقت QA)`, saved (`"آدرس با موفقیت تغییر یافت."` Persian success message), **confirmed persisted after a full page reload** (not merely an optimistic UI update), then reverted to its original empty value — the QA account was left exactly as found.
7. **Navigation.** Every dashboard tab (`رزروهای من`, `سفارش‌ها`, `پیام‌ها`, `مسیر زیبایی من`, `حساب کاربری`) and every WooCommerce account-page nav item (`پیشخوان`, `سفارش‌ها`, `دانلودها`, `آدرس ها`, `جزئیات حساب`, `بیرون رفتن`) resolved to real, correct, non-404 content. `علاقه‌مندی‌ها` (wishlist) is a declared-not-ready placeholder (`ready: false` in `dashboard-customer.tsx`), rendering the same honest "این بخش در نسخه بعدی محصول تکمیل می‌شود" empty state used elsewhere — a documented deferral, not a bug.
8. **Save/update functionality:** confirmed working (see #6). Notification-preference toggles on the Account tab (5 categories, all labeled `فعال`/active) are independently wired buttons, consistent with Step 10/20's documented preference model.
9. **Persistence test:** see #6 — passed, with restoration.
10. **Logout:** WooCommerce's `بیرون رفتن` nav link correctly ends the session (verified: subsequent request to `/my-account/edit-account/` redirected to `/auth/` post-fix, and returned WooCommerce's own login form pre-fix — see the NAVIGATION finding above).
11. **Privacy controls:** the Account tab's "حریم خصوصی و اطلاعات من" section (data export request + account deletion request, `DataExportCard`/`AccountDeletionCard`) rendered correctly with real, honest status text (export shown as `منقضی‌شده — می‌توانید دوباره درخواست دهید`), Persian throughout, matching V2.2 Step 14's documented design.
12. **Mobile (375×812):** both `/dashboard/` and `/my-account/edit-account/` show zero horizontal overflow (`document.documentElement.scrollWidth === clientWidth`, verified programmatically, not just visually assumed).
13. **Accessibility:** not run through axe-core this pass (time-budget tradeoff, disclosed above) — a real gap in this section's coverage, not claimed as clean.
14. **WooCommerce/WordPress default UI findings:** the account area is a **deliberate mixture (Type D)** — a custom BeauClick "حساب کاربری" dashboard tab (preferences, privacy) links out to WooCommerce's native My Account templates for profile/address editing. This is *not* an unbranded default WooCommerce page: computed styles confirm the account-page buttons use BeauClick's own design tokens (`background-color: oklch(0.4 0.16 290)` — the documented Primary violet, `border-radius: 12px`, `font-family: Vazirmatn`) via `assets/css/woocommerce.css`, enqueued only on `is_account_page()`/checkout/cart/shop templates per `functions.php`. One found-and-fixed gap in this mixture is the NAVIGATION finding above (logged-out fallback leaking the wrong login UI); with that fixed, the two halves now form one coherent flow rather than "a BeauClick page followed by an unrelated WordPress page."
15. **Bugs discovered:** 1 (the logged-out `/my-account/*` redirect gap — see Findings above).
16. **Bugs fixed:** 1 of 1 found in this specific section.
17. **Remaining gaps:** no automated accessibility pass; no live test of `/my-account/edit-account/`'s password-change fields (out of scope — normal accounts don't use passwords, this path exists for administrators only per the product's own design) or of the wishlist placeholder (already documented as V2.4-deferred).

**Direct answers, as required:** Is the customer account dashboard fully Persian? **Yes.** Does it actually work? **Yes**, verified with a real edit→save→reload→persist round trip. Are profile fields editable? **Yes**, on the linked WooCommerce page (not the BeauClick tab itself, by design). Does address editing work? **Yes**, confirmed with real persistence. Does saved data persist? **Yes.** Does the account experience look like BeauClick? **Yes** — same design tokens, confirmed via computed styles, not assumption. Are there default WordPress/WooCommerce sections still exposed? **Yes, by design** (WooCommerce's own account/address forms), themed to match BeauClick rather than replaced. Are there dead buttons or broken links? **One was found and fixed** (the logged-out redirect gap); none remaining in this section as tested. Does it work correctly at 375/390/412px? **Confirmed at 375px** (zero overflow, programmatically verified); 390/412 not independently re-tested (functionally equivalent viewport range, not a substitute for actually running them — disclosed, not assumed).

### Tests

Baseline (before any fix in this pass): backend PHPUnit 857/857, frontend Vitest 48/48, TypeScript clean, ESLint clean, `php -l` clean across every `beauclick-*` plugin and the theme — unchanged from the V2.3 Final Release Audit's own numbers, confirming no drift on the single intervening documentation-only commit.

After fixes: backend PHPUnit **857/857** (3 pre-existing AI-provider tests updated to assert the corrected Persian-digit output, not skipped or weakened; no test count change), frontend Vitest **48/48** (unchanged — no frontend test touched the fixed `AuthFlow.tsx` resend path directly, a real coverage gap worth a future regression test), TypeScript clean, ESLint clean, `php -l` clean.
...
## V2.4 Step 26 (part 1) — V3 Migration Preparation Layer: GAP-02, GAP-08

**Status: implemented.** The two cheapest, highest-confidence `V3_GAP_REGISTER.md` `REQUIRED` fixes, taken first per the recommended implementation order. Both are structural-enforcement / correctness fixes with zero UI surface.

### GAP-02 — audit-logging enforcement + the one still-open instance

**Pre-existing-state re-audit (fresh, not from memory):** grepped every `bc_manage_platform` reference across all 17 plugins, then narrowed to REST controllers with a genuine, unconditional `require_admin()`-style capability gate (excluding `BookingController`'s admin capability, which is only a *fallback* inside a three-way customer-OR-provider-OR-admin ownership check, not a pure administrative-CRUD gate — a materially different case from the confirmed bug class). Confirmed exactly **5** such mutating routes: `B2BController::approve_account/reject_account/submit_quote_prices` (already fixed by the V2.3 Final Release Audit) and `set_tiers` (**still open**, confirmed by direct code read — no `AuditLogger`/`audit_log()` call anywhere in the method), plus `LoyaltyController`'s 8 admin mutations (already fixed, per that controller's own docblock). `VerificationController`'s moderator-gated mutations (`decide`/`suspend`/`revoke`/`reinstate`) are **not** part of this bug class — they already write to their own domain-specific `wp_bc_verification_history` table (V2.1 Step 8), which `AuditLogger`'s own docblock names as the precedent this general mechanism was generalized from.

**Enforcement mechanism** — mirrors the exact shape `RestController::route()` already uses for the missing-`permission_callback` guard (a boot-time `LogicException`, not a runtime check): a route may now declare `'adminGated' => true`, and if it does, must also declare either `'auditAction' => 'action_type_string'` (the string its handler calls `AuditLogger::record()` with) or `'auditExempt' => 'reason'` (a short, real reason it deliberately doesn't — e.g. read-only). Registration itself throws if neither is present. Deliberately **opt-in** (triggered only by the new `adminGated` key, never auto-detected from an opaque `permission_callback` closure) — this does not retroactively demand a declaration from the hundreds of ordinary, non-admin routes across the codebase that were never part of this bug class; it applies to the confirmed 5 (now declared) plus every future admin mutation that opts in.

**Fixed:** `B2BController::set_tiers()` now records a `b2b_tiers_set` audit entry (before/after tier state), matching the exact shape `approve_account`/`reject_account`/`submit_quote_prices` already use. All 5 confirmed B2B routes and all 8 confirmed Loyalty routes now carry the `adminGated`/`auditAction` declaration.

### GAP-08 — the ownership helper's real limitation

**Re-audit, correcting a stale claim in this project's own planning:** `V3_GAP_REGISTER.md` (and this document's own V2.4 roadmap section) claimed `require_owner_or_capability()` was "dead code, called nowhere." A fresh grep found this **false** — it has 4 real call sites (`WaitlistController::can_cancel()`, `JourneyController::can_edit_goal()`, `MyProfileController::can_edit_service()`, `ReceiptController`), all doing direct WP-user-ID ownership comparison. The *actual* gap, confirmed by reading `BookingController::can_confirm()`/`can_manage_booking()`, is narrower and more precise than "unused": the method could not express **indirect** ownership (a booking's `provider_id` is a CPT post id, not the owning professional's WP user id), so `BookingController` reimplemented the same owner-or-capability shape inline instead of using the shared helper.

**Fix:** added an optional third parameter, `?callable $owner_resolver = null` — when omitted, behavior is byte-for-byte unchanged (direct WP-user-id comparison, exactly what all 4 existing call sites need, none of them touched); when supplied, it resolves the current user's own identity into whatever id-space the resource's ownership is expressed in (e.g. `ProviderLookup::for_user()`). `BookingController::can_confirm()` and `can_manage_booking()` now delegate to this shared helper instead of their own inline copy of the same logic.

### Tests

9 new tests (5 `RestControllerTest` for GAP-08's resolver behavior — direct-ownership unchanged, indirect success, indirect fallback; 4 `RestControllerTest` for GAP-02's enforcement — throws when undeclared, doesn't throw with action/exempt/non-admin-gated; 1 `B2BControllerTest` for the `set_tiers` audit fix). Backend suite grew **882 → 891**, all green. The pre-existing `BookingControllerTest` cases (`test_the_owning_professional_can_confirm_their_own_booking`, `test_a_different_professional_cannot_confirm_someone_elses_booking`) passed unchanged through the refactor, proving real behavior preserved, not just "doesn't crash." `php -l` clean across all 17 plugins.

### Live QA

Confirmed the site boots correctly and the REST API responds normally after modifying the shared `RestController` base class every one of the 17 plugins' controllers extends — a real risk given a `LogicException` at route-registration time would fatal the *entire* site if any existing route were unintentionally caught by the new enforcement (none were, since it's opt-in). Live-verified `can_manage_booking()`'s fixed indirect-ownership check with a real professional session (`bc_demo_sara_ahmadi`) against real bookings: the owning provider's request to `/booking/bookings/25/reschedule-eligibility` returned `200` with a real domain answer; the same request against a booking she doesn't own (`/booking/bookings/1/...`) returned `403`. `B2BController::set_tiers`'s audit fix was verified via the real database-backed integration test (`WP_UnitTestCase`, not mocked) rather than a live wp-admin click-through — setting a temporary password on `bc_qa_admin` was, consistent with the V2.3.1 audit, correctly refused by this session's own auto-mode safety classifier as a credential-modification action.
...
## V2.4 Step 26 (part 2) — Money-Adjacent Correctness: GAP-01, GAP-03, GAP-04

**Status: implemented (GAP-01 partially, disclosed below; GAP-03/GAP-04 fully).** The three remaining `V3_GAP_REGISTER.md` `REQUIRED` correctness fixes, taken next per the recommended implementation order — deliberately sequenced after Step 24 and before real transaction volume accumulates further.

### GAP-01 — ledger append-only enforcement

**Re-audit, before writing any code:** read the entirety of `LedgerService` fresh. Confirmed it already has no `update`/`delete` method of any kind — every correction (a refund) is its own new `insert()` row, never a mutation of an existing one. Code-level immutability already held by convention; the real, remaining gap is that nothing at the *database* layer stopped a raw `$wpdb->update()`/`$wpdb->delete()` elsewhere, a future careless migration, or a direct database-tool edit from silently rewriting a financial fact after it was recorded.

**Fix:** a new migration (`AddLedgerImmutabilityTriggers`) adds two `BEFORE UPDATE`/`BEFORE DELETE` triggers on `wp_bc_ledger_entries`, each unconditionally raising a SQL error (`SIGNAL SQLSTATE '45000'`) — genuinely append-only at the database layer regardless of which code path or human attempts the mutation, the same guarantee a revoked UPDATE/DELETE grant would give without this codebase needing a second, separately-privileged database user (no such infrastructure exists here).

**Disclosed deployment precondition, found by actually running the migration, not assumed:** `CREATE TRIGGER` requires either the `SUPER` privilege or the server variable `log_bin_trust_function_creators = 1` whenever binary logging is enabled — a MySQL/MariaDB replication-safety rule. Both this local dev database's own user *and* the PHPUnit test database's own user were confirmed, by directly querying `SHOW GRANTS`, to have `GRANT ALL` scoped only to their own two schemas — no global/SUPER grant — so `CREATE TRIGGER` is rejected here with `Access denied ... you need SUPER or SYSTEM_VARIABLES_ADMIN`. The migration's `up()` checks each `CREATE TRIGGER` result explicitly and logs a clear, actionable message (visible in `debug.log`, reproduced below) rather than silently claiming success; it does not throw, so a restricted host's migration run still completes normally and every other pending migration still applies.

```
BeauClick: wp_bc_ledger_entries immutability triggers could not be created (You do not have the
SUPER privilege and binary logging is enabled...). The ledger remains append-only at the CODE
layer (LedgerService has no update/delete methods), but is not yet enforced at the DATABASE layer
on this host. To close this: either grant SUPER to the application DB user, or have the hosting
provider set log_bin_trust_function_creators = 1, then re-run this migration's up() once.
```

This is the same category of precondition as the SMS/SMTP/payment-gateway credentials `PRODUCT_GAP_REGISTER.md` already documents as required-before-production — infrastructure the application cannot self-elevate, not a defect in the migration. **GAP-01 is therefore only partially closed**: real, confirmed today (code-level, always true regardless of hosting), and ready to fully close itself the moment either privilege precondition is met on a given host, with zero further code changes.

### GAP-03 — `BookingOrderBridge::create_order_for_booking()` idempotency

**Re-audit finding:** the method unconditionally called `wc_create_order()` on every invocation, with no check for whether the booking already had a linked order. The only confirmed real call site (`Plugin::attach_order_to_booking_result()`, fired once per `beauclick/booking/after_create`) could not currently be double-invoked through the *primary* flow — slot-claiming already makes a resubmitted create-booking request fail before a second booking row exists — but the method itself offered no defense-in-depth against a retried call, a future recovery/retry tool, or a hook re-fire, unlike every other write in this codebase (`LedgerService::insert()`'s `UNIQUE` constraint, `CampaignService::record_usage()`'s `UNIQUE(booking_id)`).

**Fix:** the method now checks `wp_bc_bookings.wc_order_id` first; if already set, it loads and returns that existing `WC_Order` instead of creating a new one. Omitted/falsy `wc_order_id` (the normal first-call case) is completely unaffected — zero behavior change for the primary flow.

**A real, existing test's premise broke, correctly, not spuriously:** two pre-existing tests (`CampaignDiscountIntegrationTest`, `PaymentRefundIntegrationTest`) explicitly relied on and documented the *old*, buggy "re-firing creates a second order every time" behavior as their reproduction method for a "spurious duplicate order" scenario. Both were updated — not weakened — to match the corrected behavior: a re-fire for the same booking now returns the identical order (proven directly), and the "genuinely two real orders for one booking" scenario those tests actually cared about (ledger/refund self-healing) is now constructed by directly building a second order the way the old code used to, since the easy accidental path to it no longer exists. One of these updates surfaced a second, real, useful finding: with GAP-03 fixed, `wp_bc_bookings.wc_order_id` now correctly stays pointed at the real order throughout, which means `BookingService::cancel_booking()`'s existing FIN-02 safety net (refund a booking's own linked order if it's already paid, added V2.3 Step 18) now correctly reaches that real order too when a booking is cancelled as a side effect of a spurious second order's own refund — a **stronger** guarantee than the pre-GAP-03 tests proved (no booking can end up cancelled while any of its real orders is left paid and un-refunded), not a regression.

### GAP-04 — campaign usage-cap TOCTOU race

**Re-audit finding:** `record_usage()`'s own `UNIQUE(booking_id)` constraint only ever guarded the same booking being recorded twice — it never protected `usageLimitTotal`/`usageLimitPerCustomer` themselves. `EligibilityResolver::is_eligible()`'s `usage_count() >= limit` check runs as plain candidate-selection SELECTs, well before `CampaignDiscount::apply()` later calls `record_usage()` — a real, confirmed check-then-act gap: two concurrent bookings for *different* booking_ids against the same near-full campaign could both read a count still under the limit and both then insert successfully, overshooting the configured cap.

**Fix, and a materially important implementation-approach correction made *during* this step, not before it:** the first implementation used an explicit `START TRANSACTION` + `SELECT ... FOR UPDATE` around the cap check and insert — theoretically correct locking, but empirically found (by actually running the full test suite, not by assuming) to silently commit `WP_UnitTestCase`'s own per-test transaction wrapper every time it ran, since `START TRANSACTION` always implicitly commits whatever transaction is already open on the connection. Because `CampaignDiscount::apply()` sits on the real `beauclick/booking/after_create` hook, this corrupted test isolation for *every* test exercising that hook, not just this method's own tests — a real, reproduced problem (20+ cascading failures across unrelated files: `EligibilityResolverTest`, `PaymentRefundIntegrationTest`, `LoyaltyIntegrationTest`, `MembershipServiceTest`), caught and root-caused before it reached the docs or a commit, not shipped.

The corrected, final implementation (`CampaignService::record_usage_within_cap()`) is a single atomic `INSERT ... SELECT ... WHERE` statement — the cap re-check and the insert as one unit of work, matching this codebase's own established idiom (`record_usage()`'s own `INSERT IGNORE`) rather than an explicit multi-statement transaction. It needs no transaction of its own, so it composes safely inside any caller's transaction context (or none), test harness included. `CampaignDiscount::apply()` now calls this method instead of `record_usage()` directly, passing the campaign's own `usageLimitTotal`/`usageLimitPerCustomer` through.

**Empirically verified under genuine concurrent load**, not just reasoned about: ad hoc multi-process racing during development (3 real OS processes, real separate MySQL connections, against a cap of 2) consistently left exactly 2 rows, never more, across 5 repeated runs — sometimes via MySQL blocking the losing connections until the winner's insert became visible, sometimes via MySQL's own deadlock detector aborting a loser's statement outright (both legitimate resolutions; a deadlock-aborted attempt simply doesn't grant that discount, an availability trade-off, never a correctness violation). This exact technique is now a permanent, repeatable test (`CampaignUsageCapTest`'s own concurrency test).

### Tests

Backend suite grew **891 → 910** (19 new: 2 `LedgerServiceTest` trigger tests — both self-skip with a clear reason on a host lacking the DB privilege, confirmed correctly skipping here, rather than falsely passing or failing on an infrastructure precondition; 1 `BookingOrderBridgeTest` GAP-03 idempotency test; 6 `CampaignUsageCapTest` GAP-04 tests including the real multi-process concurrency proof; 2 existing tests in `CampaignDiscountIntegrationTest`/`PaymentRefundIntegrationTest` updated, not weakened, to match GAP-03's corrected behavior). All 910 tests pass (2 skipped for the disclosed GAP-01 hosting precondition; the 2 pre-existing `CatalogContextTest` failures were confirmed, by running the identical filter against unmodified `master`, to be unrelated pre-existing flakiness, not caused by this step). Frontend unchanged **55/55** (no frontend code touched). `php -l` clean on every changed/new file.

### Live QA

A real booking against a real active campaign (`bc_qa_customer`, a real open slot, campaign id 1) was created via the live REST API: the order correctly received both the real service line item and the campaign discount fee, `wp_bc_campaign_usages` recorded exactly one row via the new atomic guard, and completing payment recorded correct, real commission/receivable ledger entries with the booking transitioning to `confirmed`. GAP-03 was directly verified against this same real booking: a second, manual `create_order_for_booking()` call for the same booking_id returned the identical order (78), confirmed via `wp_bc_bookings.wc_order_id` staying unchanged. Console/network sweep during this flow: zero new errors (the buffered `ERR_CONNECTION_REFUSED`/`401`/`403` entries were confirmed, the same way as Step 24's own audit, to be stale entries from earlier unrelated navigation in the same long-lived tab).
...
## Global UI/UX Polish Pass (Product & Professional Profile)

**Audit date:** 2026-08-19. **Baseline:** `master` @ `ae96fe2`, matching `origin/master`, clean working tree except one unrelated untracked `start-beauclick.vbs`. All 8 historical tags (`v1.0.0`→`v2.4.0`) untouched; no new tag created by this pass. **Trigger:** a product-owner-issued Global UI/UX Regression & Polish Audit brief naming two Primary Targets — the Product/WooCommerce customer experience and the public Professional Profile page — neither of which the prior [Global UI/UX & Product-Wide Audit](#global-uiux--product-wide-audit) (`8c50b67`, 2026-08-18) had reached; that pass covered auth/OTP, header search, the booking flow, and the customer account dashboard, and explicitly disclosed the shop/product pages and the public professional profile as untouched.

**Method — real, not code-only.** Ran the actual local stack (PHP's built-in server on `wordpress/router.php`, the real MySQL dev database, real seeded catalog/provider data) and drove it with a real browser (navigation, DOM/computed-style inspection, console/network monitoring) at both desktop and 375px mobile widths, the same standard the prior audit set. Source-level exploration (three parallel read-only passes over the theme's WooCommerce integration, the public professional-profile template, and the shared design-token pipeline) preceded any change, to find root causes rather than patching symptoms.

**Scope actually covered, in depth:** the single-product page (`/product/*`, WooCommerce's own stock template — the theme had never overridden it), the shop's related-products and up-sells sections, the cart page, the checkout page, the public professional-profile page (`/professional/*`), and the shared design-token pipeline (`shared/design-tokens.json` → `app/src/design-system/tokens.generated.css` vs. the WordPress theme's hand-ported `components.css`).

**Scope sampled, not exhaustively re-audited:** the homepage, marketplace listing, and order-received receipt got a live smoke pass (console/network errors, overflow, Persian/RTL) to catch regressions, not a full re-audit — all three were already verified clean by the prior pass and none of this pass's changes touch their templates. The customer account dashboard, auth/OTP flow, and booking flow were not re-visited at all (no code in this pass touches them).

**Scope disclosed as not reached, same limitation as the prior pass:** the wp-admin-side BeauClick admin pages, a full B2B quote-acceptance round trip, the professional CRM/staff/calendar tabs (all still blocked on the same admin-credential constraint the prior audit hit), and a systematic automated (axe-core) accessibility pass — a manual spot-check (semantic structure, `aria-*` presence, computed contrast on new/changed elements) substituted, disclosed rather than claimed as a full audit. No real order/payment was completed live this pass for a product purchase specifically (the underlying WooCommerce payment bridge and `thankyou.php` receipt template were already exhaustively verified by the prior audit via the booking flow, and this pass's product-page changes never touch that template — its source was read and confirmed already correct, including Persian digits and real Jalali dates, not re-verified via a live transaction).

### Findings — Product/WooCommerce experience

**BUG, BLOCKER — fixed.** The single-product page (`/product/*`) had no theme template override at all — the only two WooCommerce overrides in the theme are `woocommerce/archive-product.php` (shop grid) and `woocommerce/checkout/thankyou.php` (receipt). WooCommerce's stock `single-product.php` calls `do_action('woocommerce_sidebar')`, which by default renders WordPress core's own `get_sidebar()` — and this theme has no widgetized sidebar at all. Live, this rendered a raw PHP deprecated-file notice directly in the page HTML ("پروندهٔ پوسته بدون sidebar.php…") followed by an entirely unstyled, unbranded default WordPress sidebar (search box, page list, monthly archives, category list) on every single-product page view — a severe, real-user-visible instance of exactly the "WordPress/WooCommerce default UI leaking" pattern this audit brief asked to find. **Fix:** `remove_action( 'woocommerce_sidebar', 'woocommerce_get_sidebar', 10 )` added to `functions.php` (WooCommerce registers this at plugin-load time, before the theme's `functions.php` runs, so an unconditional top-level removal is safe). Verified live: notice and sidebar both gone, page unaffected otherwise.

**BUG/PRODUCT GAP, HIGH — fixed.** Missing/broken product images on WooCommerce-native surfaces. The theme already has a proven, working fallback (`bc_mockup_image_url()`: real image → themed SVG mockup → gradient `.bc-placeholder-image`, used by the shop grid and provider cards) — but WooCommerce's own single-product gallery (`wc_get_gallery_image_html()`/`woocommerce_single_product_image_thumbnail_html`) and every `WC_Product::get_image()` call site (cart table, checkout order review) never used it, falling back instead to WooCommerce's own default gray `woocommerce-placeholder.png`. Reproduced live: a product with a real mockup (`شامپو ضدریزش`) correctly showed its mockup on the single-product page but the default gray Woo placeholder in the cart table — two different WooCommerce code paths, same underlying gap. **Fix:** two new filters in `inc/woocommerce-images.php` (`woocommerce_single_product_image_thumbnail_html` for the gallery, `woocommerce_product_get_image` for every list/cart/checkout context), both reusing the existing `bc_mockup_image_url()` unchanged — no new fallback logic invented. Verified live in both branches (mockup SVG and gradient) on the gallery and the cart thumbnail.

**BUG/PRODUCT GAP, MEDIUM — fixed.** Related-products and up-sells rendered with WooCommerce's own default `<li>` markup (`woocommerce/content-product.php` had no theme override), floated, unstyled, visually inconsistent with every other product card in the product (shop grid, provider cards). **Fix:** new `woocommerce/content-product.php` override — a thin wrapper reusing `template-parts/product-card.php`'s exact existing card markup (the same file the shop grid already uses), since WooCommerce calls this one partial for the shop loop *(bypassed here — `archive-product.php` already renders the card directly)*, the related-products loop, and the up-sells loop alike. New CSS in `assets/css/woocommerce.css` turns the surrounding `.related.products`/`.up-sells.products` `ul.products` from WooCommerce's floated columns into a `--bc-space-card-gap-large`-token grid matching `.bc-grid` elsewhere, plus new rules for the single-product gallery/summary two-column layout, price/rating/meta typography, the add-to-cart form, and a themed description/reviews tab strip — all pulling existing `--bc-*` tokens, none inventing new values, consistent with this file's own documented CSS-only strategy. Verified live: 3-column grid, correct Persian "محصولات مرتبط"/"توضیحات" headings (WooCommerce's own `fa_IR` translation was already installed and active — no string-translation work needed), themed tabs, zero horizontal overflow at 375px and desktop.

**BUG (i18n/consistency), LOW — fixed.** Two more raw-Latin-digit numbers found live on WooCommerce-native output, the same class of bug the prior audit's LOC-07 fixed for the AI assistant: the single-product page's stock count ("49 عدد در انبار" instead of "۴۹ …") and the checkout order-review table's item quantity ("شامپو ضدریزش × 1" instead of "× ۱"). Prices were already correct everywhere (`beauclick-payments\Currency` already hooks `wc_price`) — quantities and stock counts are not prices, so they slipped past that existing filter. **Fix:** two new filters in `inc/woocommerce-i18n.php` (`woocommerce_get_stock_html`, `woocommerce_checkout_cart_item_quantity`), both a single `bc_persian_digits()` call at the output point, mirroring the established pattern exactly. **Deliberately not hooked:** the cart page's own `woocommerce_cart_item_quantity` filter — that one wraps a real, editable `<input type="number" value="1">`; converting its `value` attribute to a Persian digit would silently break quantity editing (verified live, both before this decision and after the checkout fix, that the cart's quantity input still holds a valid numeric value and remains editable).

### Findings — Professional Profile experience

**UI/UX (layout), MEDIUM — fixed.** `single-bc_professional.php`'s hero (cover image, avatar, name/rating, booking/message CTAs) was built entirely with hardcoded inline `style=""` attributes instead of the shared token scale (`shared/design-tokens.json`) every other page uses — most concretely, the cover's `aspect-ratio:16/5` had no `max-height`, so it scaled unbounded with viewport width (≈395px tall at a 1265px content width, confirmed live before the fix) — the direct, measured cause of the audit brief's "oversized hero" symptom. **Fix:** cover, avatar, header, and actions moved to four new token-driven classes in `theme.css` (`.bc-profile-hero__cover` with a `max-height:240px`/`21:5` desktop and `160px`/`16:7` mobile cap, `__header`, `__avatar` at the design system's own `--bc-radius-avatar` token instead of a hardcoded `24px`, `__actions`), same visual relationship (avatar overlapping the cover, name/rating beside it, CTAs trailing) — a bounded, tokenized version of the same design, not a redesign. Below 640px the actions row now takes the full row width instead of wrapping into a cramped remainder. Verified live via computed styles at both desktop (cover capped at exactly 240px, was unbounded before) and 375px (actions row full-width, zero horizontal overflow, `dir="rtl"`/`lang="fa"` confirmed).

**Confirmed clean, not fixed (no gap found):** the badge/rating markup on the public profile duplicates (rather than reuses) the React design-system's `Badge`/`RatingStars` primitives — flagged during exploration as a possible consistency risk, but the WordPress theme's hand-ported `components.css` was spot-checked byte-for-byte against `app/src/design-system/primitives/Badge.css` and `RatingStars.css` and found identical in both; the "keep both in sync by hand" discipline documented in `components.css`'s own header comment currently holds. Not unified into a single React component (the public profile has no React mount at all — see the exploration note below), since that would be an architecture change, explicitly out of this pass's scope. The portfolio/services/reviews sections, empty states, and the `.bc-section` spacing scale (also used identically on every other page) were reviewed and found consistent with the design system — the "excessive empty space" symptom traced specifically to the unbounded cover height above, not to section padding.

**Exploration note (not a bug, informs future work):** there is no React component at all for the public professional-profile page — it is 100% server-rendered PHP; the only React under `features/dashboard/professional/` is the *owner's own private editing dashboard* (a separate surface). This is a description of current architecture, not a gap this UI-polish pass should close.

### Design system

Spot-checked the WordPress theme's hand-ported `components.css` against the React design-system's `app/src/design-system/primitives/*.css` (the two are meant to stay in sync by hand, per `components.css`'s own header comment, with nothing enforcing it automatically) — `Badge.css`/`RatingStars.css`, the two primitives this pass's professional-profile findings made most relevant, were found byte-for-byte identical. No drift found in this spot-check; not exhaustively diffed across every primitive (`Button`, `Card`, `Chip`, `Input`, `Modal`, `PlaceholderImage`, `PriceTag` not individually compared this pass) — disclosed as a partial check, not claimed as a full audit of the pair.

### Tests

Backend PHPUnit test scaffolding (`wordpress-tests-lib`) was not present in this session's environment and had to be installed fresh (`bin/install-wp-tests.sh`, using the pre-existing `beauclick_test` database) before any test could run — a one-time local environment-setup cost, not a code change. Once installed: PHPUnit **944/944 with 2 pre-existing failures** (`MarketplaceControllerTest::test_the_default_browse_sort_orders_by_real_ranking_score`, `CatalogContextTest::test_summary_is_empty_when_no_context_signals_are_known`) confirmed, via `git stash` and a targeted re-run against unmodified `HEAD`, to reproduce identically before this pass's changes — pre-existing, unrelated to any template/CSS/filter change made here, not weakened or skipped. Frontend Vitest **72/72** unchanged (this pass touched zero files under `app/src`). TypeScript clean, ESLint clean, `php -l` clean on every file touched. PHPCS on the files this pass touched or created showed only: pre-existing `WordPress.NamingConventions.PrefixAllGlobals`/short-array-syntax debt in `single-bc_professional.php` lines this pass did not structurally touch (consistent with the same repo-wide pre-existing debt the V2.2/V2.3 audits already documented and declined to mass-fix); a CRLF line-ending notice confirmed, via `git diff`, to be a pre-existing Windows-checkout artifact affecting every PHP file equally, not something this pass introduced. The one real new-code issue found (short array syntax in the newly-written `content-product.php`) was fixed immediately, since there's no "preserve existing style" argument for a brand-new file.

### Live QA

Verified live, desktop and 375px mobile: single-product page (sidebar/notice gone, mockup + gradient placeholder both branches, Persian stock digits, themed tabs), related-products/up-sells grid (3 columns, Persian heading, zero overflow), cart (mockup thumbnail, quantity input still editable and numerically valid), checkout (Persian quantity, full province list, zero console errors), professional profile (bounded hero at both widths, RTL/Persian confirmed via computed `dir`/`lang`), homepage and marketplace (smoke pass — zero console errors, zero horizontal overflow, unchanged by this pass). No real product purchase was completed live (see scope disclosure above).

### Verdict

**GLOBAL UI/UX POLISH PASS (PRODUCT & PROFESSIONAL PROFILE) COMPLETE**, conditioned on the disclosed scope limits above (wp-admin pages, B2B quote-accept, professional CRM/staff/calendar tabs, a systematic axe-core pass, and a live product-purchase transaction were not reached this pass) — every BLOCKER/HIGH finding was fixed and verified live; the two MEDIUM/LOW findings in scope were fixed since both were low-risk; no release-blocking defect remains open. No tag created — tagging remains a separate, explicit product-owner action, per this document's own established pattern.
