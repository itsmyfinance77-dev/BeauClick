...
**Amended 2026-09-15 (Story #160 implementation) — six engineering consistency notes on §6, inside the ratified structure.** The read-only readiness audit of #160 against `b71f10cbb524e687243f53b5b63812eee80ae9a8` and the implementation preflight found places where §6 and its lock table could not be satisfied literally together with `V33-DEC-039`, ADR-050 or the merged code. Each is resolved by the rule a binding record dictates; none changes a ruling, an owner decision, a value or a customer-visible entitlement, and the original sentences below are preserved and read with these notes. **(1) The timeliness instant is the cancellation's, not the decision's.** §6 said `timely ⇔ decidedAt <= slotStart − cutoffHours` with `decidedAt` read inside the deciding transaction, and placed that transaction on the asynchronous `BookingCancelled` consumer. `V33-DEC-039` R4 compares the *cancellation* against the database clock *inside the cancelling transaction*; a consumer that runs later — after a relay delay or a retry caused by any sibling handler — would have turned a timely cancellation late. The implemented rule compares `booking.booking_history.created_at` of the booking's `cancelled` row (the cancelling transaction's `now()`) with `slot_start − cutoff_hours`, in SQL; that instant is copied by value into the decision as the added column `event_instant`, and `decided_at` remains the audit instant. **(2) Order row before booking row, on every new path.** §6 and the lock table's reschedule row wrote booking `FOR UPDATE` → order `FOR UPDATE`, the reverse of ADR-050 §4.2/§7 and of the live confirmation path (order capture → booking confirm). The cancellation consumer takes order `FOR UPDATE` → booking `FOR SHARE`, and a governed reschedule takes order `FOR UPDATE` → booking `FOR UPDATE` before claiming a slot; the reschedule path previously held no explicit booking lock at all, only its compare-and-swap. **(3) Two columns move forward; the rest are frozen.** §6 called `commerce.booking_outcome_decisions` "append-only by trigger" while requiring `execution_status` to be recorded after the refund and `superseded_by_id` to be set by a later row. The trigger freezes every column except `execution_status` (`pending → executed | manual_required | failed`, once) and `superseded_by_id` (`NULL → id`, once) and refuses `DELETE`. **(4) A retry reports, it does not re-execute.** §6 and *Failure semantics* said a retry "re-calls `refund` with the same key"; `PaymentService.refund` returns the existing row for a known `(order_id, request_key)` whatever its status, so a `failed` or stuck `pending` refund is reported truthfully and is not re-driven by this ADR's mechanism (re-driving a real refund is #47). **(5) The non-zero reschedule consequence is dormant.** `V33-DEC-039` R8 evaluates a later or post-cutoff reschedule "as if it were a cancellation of the old slot followed by a new booking", but no ratified record says whether a retained amount is refunded, carried forward as the new slot's prepayment, or what the immutable schedule's venue balance then means. Only the zero-money consequence is implemented (shown first, confirmed with `acceptConsequence`); a non-zero consequence is refused and a CHECK makes a `reschedule_consequence` row carrying money unwritable, until those semantics are ratified — reachable only after Legal evidence exists. The free count counts only the customer's own reschedules, from booking history. **(6) One captured payment is refunded once.** The preflight reproduced on real PostgreSQL that a booking cancelled while its payment was being captured received two full refunds, under `booking-unconfirmable:<orderId>` (the capture compensation) and `booking-cancelled:<bookingId>` (the consumer), in either order. The compensation now uses the cancellation's own key when the booking it could not confirm was cancelled, and the decision computes the collected remaining amount net of the order's non-failed order refunds already issued, projected or not. Additionally recorded, not a correction: a `force_majeure` cause exists in the vocabulary with no producer — the administrator declaration of `V33-DEC-039` R8 is allocated to no child by the mechanism map below — and `no_show` / `dispute_outcome` decision rows stay unwritable (`ck_bod_kind_defined`) until #161 / #162 define their rules.
