...
Status: **Business/Seller domain, Waitlist domain, and the financial outbox consumer are complete, tested, and verified against real PostgreSQL by CI — 341 of 348 real-Postgres/OpenSearch tests passing, the remaining 7 all one pre-existing, investigated, deliberately-untouched Phase 1 artifact** (see §16 for the exact verification chain, the eleven real bugs found and fixed along the way, and what "verified by CI" means here, since this phase's own local environment could not run that suite directly). GAP-06 (real payment gateway) remains genuinely external. RBAC/audit expansion beyond what Business/Waitlist themselves needed is DEFERRED on evidence — see §21.
...
Phase 3's report claimed no remote runner existed. That was checked for the first time this phase, via `gh run list`, and was **false**: a real GitHub Actions workflow has been running on every push since Phase 3's CI landed, and had been **failing on every single one**, unnoticed. Getting it to a genuine, verified pass took **eleven** real, ordered fixes, each one exposing the next problem underneath it — recorded here in full because the sequence itself is the honest evidence this was debugged for real against CI's own failure output, never asserted fixed on the strength of local reasoning alone:
...
7. **Two genuine application dependency-injection bugs**, only reachable once the pipeline got far enough to actually boot the app: `WaitlistProfessionalResolver`'s port was declared but never bound in the composition root, and `NotificationEnricher`'s new `BusinessEntity` dependency was never registered in the specific module (`Phase3CompositionModule`) that constructs it. Both broke `AppModule` boot entirely — every real-Postgres suite failed identically (~330 failing test cases, one root cause each time, confirmed by an identical error message across all of them, not guessed from the failure count alone).
8. **A third DataSource-wiring bug**, one layer deeper than DI: `TypeOrmModule.forFeature()` registers a repository *provider* for a module's own injector, but does not add the entity to the DataSource's own metadata — that only happens via the `entities` array passed to `TypeOrmModule.forRootAsync()`, which never listed `BUSINESS_ENTITIES`/`WAITLIST_ENTITIES`. Every order creation (which now asks `SellerPartyLookup.forProfessional()` for the real seller party) threw `EntityMetadataNotFoundError`, taking down `booking-lifecycle`, `financial-integrity`, `payment-security`, `search-projection`, `correlation`, and `outbox-transactional` alongside the new suites — invisible to the fast pg-mem suite, which constructs its own isolated DataSource directly and never goes through this root config at all.
9. **A real bug in `WaitlistService.offerNextFor()` itself**, found from `OutboxRelay`'s own "Outbox dispatch failed" log line (a swallowed-by-design error the relay logs but does not rethrow — the actual diagnostic signal was in CI's raw log output, not in any test's own failure message): the combined `UPDATE ... WHERE id = (SELECT ... FOR UPDATE SKIP LOCKED) RETURNING id` statement's result row did not shape into `{id: string}` the way the code assumed — `rows[0].id` came back `undefined` (confirmed directly: TypeORM's `EntityNotFoundError` logged `"where": {}`, exactly what `{id: undefined}` serializes to). Rewritten as two proven steps: a plain top-level `SELECT ... FOR UPDATE SKIP LOCKED` (the same `manager.query()` read pattern `LedgerService.sumAmount()` already uses successfully) whose row lock is held for the rest of the transaction regardless, followed by a query-builder `UPDATE` with an explicit CAS.
10. **A test bug**, not an application bug, in `business-authorization.pg-spec.ts`: `invited.body.id` instead of `invited.body.data.id` (every response is wrapped in a `{data: ...}` envelope). The resulting `undefined` produced a literal `/accept` URL segment of `"undefined"`, correctly rejected by Postgres as invalid `uuid` syntax — a 500 where the test expected a 404. The stack trace terminating inside the ownership resolver's own `findById()` call is exactly correct behaviour for a garbage id; the bug was in what the test sent, not what the API did with it.
11. **A real, pre-existing gap in `BookingService.create()`'s idempotency-key handling** (Phase 2 code, untouched by every prior phase's own testing until this was the first time this exact real-Postgres test ever ran): under N>2 simultaneous identical-idempotency-key requests, only one can ever win the slot claim; every loser throws `SlotUnavailableException` directly out of `claimSlot()` (a plain JS exception, no `.constraint`) without ever reaching the idempotency-key insert whose collision the replay check was watching for — so N−2 of the losers got no idempotency protection at all. Reproduced twice in a row (a deliberate re-run, not assumed flaky) before fixing it by widening *when* the replay check runs rather than *what* it does. See PHASE4-04 in the gap register.

**What "verified by CI" means here, precisely.** This session had no working local PostgreSQL credentials (a native install exists and is running, but its password is unknown to this session; Docker Desktop was not running) and did not create either — resetting a local superuser's credentials or standing up a fresh Docker daemon were both judged out of proportion to fixing CI itself, and CI's ephemeral, disposable Postgres containers are the more representative target anyway. Every `*.pg-spec.ts` file added this phase (`waitlist-concurrency`, `business-authorization`, `financial-outbox-consumer`) was written to the same `describeIfPg`-gated convention as every existing one, verified to compile and load cleanly via `jest --config apps/api/jest.pg.config.js` without a database (confirming no syntax/import errors), and then genuinely exercised by CI's real-PostgreSQL job. The eleven-fix cycle in this section **is** that verification: every fix was diagnosed from CI's own real failure output — log lines, stack traces, and exact error messages quoted above — never predicted or asserted fixed without a green re-run confirming it.

**Final, actual result: 341 of 348 real-PostgreSQL/OpenSearch tests passing.** The remaining 7 are ALL `financial-role-contract.pg-spec.ts` — a single pre-existing Phase 1 artifact, investigated (not merely skipped): its own setup script, `database/scripts/financial-role-contract.sql`, opens with `DROP ROLE IF EXISTS beauclick_financial_writer/reader` — the SAME role names the real, CI-provisioned `financial-roles.sql` also creates. Wiring both into the same CI provisioning step would let one script's `DROP ROLE` tear down the other's roles mid-run — a genuine, confirmed collision, not a hypothetical one, and clear evidence this script was built for a standalone, disposable environment from before financial-service existed, never meant to coexist with the real one. The guarantee it was written to prove (GAP-01) is independently and completely proven by `financial-integrity.pg-spec.ts` against the real `financial` schema instead. Deliberately left unfixed rather than patched around: see PHASE4-02 in the gap register for why a same-session hack here would trade a known, understood gap for a real risk to the tests that already pass.
...
Continued the V3 standard everywhere code allowed it: real PostgreSQL (via CI, per §16), real event contracts, real API. New tests: 2 business unit-test files (18 cases, pg-mem), 1 waitlist unit-test file (13 cases, pg-mem — `offerNextFor()` deliberately excluded, since pg-mem cannot parse `FOR UPDATE SKIP LOCKED` at all), 3 new real-PostgreSQL suites (`waitlist-concurrency`, `business-authorization`, `financial-outbox-consumer`). **Final CI result: 341/348 real-Postgres/OpenSearch tests passing** — every test this phase added or touched is green; the 7 remaining failures are entirely the one pre-existing, investigated `financial-role-contract.pg-spec.ts` gap (§16, PHASE4-02), left open on purpose rather than patched around a real role-name collision risk. See §16 for exactly what "passing in CI" is grounded in and the eleven real bugs (three pre-existing, found only because this was the first real run; the rest introduced and caught within this same phase) diagnosed from CI's own output along the way.
