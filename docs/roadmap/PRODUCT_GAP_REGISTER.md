...
## 56. V2.4 Step 25 — Analytics Benchmarking + Event Formalization Completion Note

| ID | Item | Disposition |
|---|---|---|
| — (no benchmarking of any kind existed anywhere) | Professional analytics showed only a professional's own numbers, nothing to compare against | **IMPLEMENTED** — `MetricsService::platform_benchmark()`, scoped to real same-specialty peers (never a whole-platform average, which would mix unrelated categories into a misleading figure), honestly labeled `specialty_peers`/`platform_wide`, using the identical conversion-rate definition as "your own numbers" so the comparison is apples-to-apples. `AnalyticsTab.tsx` gained a real comparison card with an honest empty state and honest "—" for missing peer-review data, never a fabricated number. |
| GAP-07 (partial) | No formal event contract exists — `event_type` documented only in a code comment, no schema, no versioning, no producer/consumer registry | **PARTIALLY_RESOLVED** — real PHP `EventLogger::EVENT_TYPES` constants replace the docblock-only list, plus a soft, `WP_DEBUG`-only `_doing_it_wrong()` notice for an unregistered event_type (never blocking the write). Deliberately smaller than GAP-07's own full V3-scale versioned contract/registry, which remains open. The registry itself was compiled by finding every real call site fresh (including indirect ones via `transition()`/`log_event()`/`MembershipService::log()` wrappers), not by trusting the prior, since-confirmed-incomplete docblock list — and this exact mechanism caught two real gaps (`membership_activated`, and the `crm_opened`/`ai_assistant_opened`/`journey_opened`/`referral_link_shared` family) during this step's own development, confirmed by running the full test suite, not assumed complete. |
...
## 58. Global UI/UX Polish Pass (Product & Professional Profile)

Product-owner-issued UI/UX regression & polish audit targeting the two surfaces the prior [Global UI/UX & Product-Wide Audit](#49-global-uiux--product-wide-audit-findings) (§49) explicitly disclosed as not reached: the Product/WooCommerce customer experience and the public Professional Profile page. Full narrative, method, and scope disclosure in `VERSION_2_ARCHITECTURE_PLAN.md`'s "Global UI/UX Polish Pass (Product & Professional Profile)" section; this entry records only the register-relevant disposition.

**Fixed (found and closed by this pass's own commit):**

| ID | Item | Disposition |
|---|---|---|
| UI-02 | Single-product page had no theme template override; WooCommerce's stock `single-product.php` called `do_action('woocommerce_sidebar')`, which rendered WordPress core's default `get_sidebar()` — a raw PHP deprecated-file notice printed into the page, followed by an entirely unstyled, unbranded default sidebar (search/pages/archives/categories) — on every single-product view | **FIXED** — `remove_action('woocommerce_sidebar', 'woocommerce_get_sidebar', 10)` added to `functions.php`. |
| UI-03 | WooCommerce's own single-product gallery and every `WC_Product::get_image()` call site (cart, checkout review) fell back to WooCommerce's default gray `woocommerce-placeholder.png` instead of the theme's own proven `bc_mockup_image_url()` fallback (mockup → gradient placeholder) already used by the shop grid and provider cards | **FIXED** — two new filters in `inc/woocommerce-images.php` (`woocommerce_single_product_image_thumbnail_html`, `woocommerce_product_get_image`), reusing the existing helper unchanged. |
| UI-04 | Related-products/up-sells rendered with WooCommerce's default floated, unstyled `<li>` markup — no `content-product.php` override existed | **FIXED** — new `woocommerce/content-product.php` reusing the shop grid's existing card partial; new grid/typography/tab CSS in `assets/css/woocommerce.css`, token-driven throughout. |
| LOC-08 | Single-product stock count ("49 عدد در انبار") and checkout order-review item quantity ("× 1") rendered in raw Latin digits — the same class of gap LOC-07 (§49) fixed for the AI assistant; prices were already correct via the existing `wc_price` hook, quantities/stock aren't prices so they slipped past it | **FIXED** — two new filters in `inc/woocommerce-i18n.php`, both a single `bc_persian_digits()` call at the output point. Deliberately did **not** hook the cart page's own editable quantity `<input>` — verified that would have broken quantity editing. |
| UI-05 | Professional-profile hero (`single-bc_professional.php`) used hardcoded inline styles instead of the shared token scale; the cover's `aspect-ratio:16/5` had no `max-height`, scaling unbounded with viewport width (≈395px tall measured live at 1265px content width) — the audit brief's "oversized hero" symptom | **FIXED** — four new token-driven CSS classes (`.bc-profile-hero__cover/header/avatar/actions`) in `theme.css`; cover capped at 240px desktop / 160px mobile, avatar radius corrected to the design system's own `--bc-radius-avatar` token, actions row goes full-width below 640px. Same visual relationship, not a redesign. |

**Confirmed clean, not fixed (no gap found):** the WordPress theme's hand-ported `components.css` was spot-checked against `app/src/design-system/primitives/Badge.css`/`RatingStars.css` (the two primitives this pass's findings made most relevant) and found byte-for-byte identical — no drift, not exhaustively diffed across every primitive. The order-received receipt template (`checkout/thankyou.php`) was source-reviewed and confirmed already correct (real Jalali dates, Persian digits throughout) — not touched, not re-verified via a live transaction. Homepage and marketplace got a live smoke pass (zero console errors, zero overflow) — unchanged by this pass, not re-audited.

**Scope disclosed as not reached this pass, not claimed as audited clean:** wp-admin pages, a full B2B quote-acceptance round trip, and the professional CRM/staff/calendar tabs — same admin-credential constraint §49 hit; a systematic axe-core accessibility pass (manual spot-check substituted); a live product-purchase transaction (the payment bridge and receipt template were already exhaustively verified via the booking flow in §49, and this pass's changes never touch that template).

**Tests:** backend PHPUnit **944/944 with 2 pre-existing failures**, confirmed via `git stash` + targeted re-run against unmodified `HEAD` to be pre-existing and unrelated (not introduced by this pass, not weakened). Frontend Vitest **72/72** unchanged (zero `app/src` files touched). TypeScript/ESLint/`php -l` clean. One real new-code PHPCS issue (short array syntax in the newly-written `content-product.php`) fixed immediately; all other PHPCS output confirmed pre-existing (untouched lines / repo-wide CRLF checkout artifact, verified via `git diff`).

**No other item in this register changed status as a result of this pass.**
