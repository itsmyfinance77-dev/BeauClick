...
No PHP/MySQL/Composer/Docker found on this machine at the time this section was first written — Node v22.20/npm 10.9.3/Git were already present and sufficient for the frontend half. PHP + MySQL were installed later (via Laragon, for its bundled MySQL) and this section's setup outline below reflects what was actually verified working, not just the original recommendation.

**Recommendation: Laragon**, for PHP + MySQL/MariaDB + Composer bundled in one Windows installer — simpler than XAMPP, less GUI-locked than Local (WP Engine's tool). **Docker is explicitly not recommended for local dev** here — nothing in this stack needs container parity yet, and Docker Desktop's WSL2 overhead on Windows is real cost for no current benefit; it can be introduced later for CI/staging parity if the team grows, without changing the app architecture at all.

**Serving WordPress: PHP's own built-in dev server, not Laragon's Apache/Nginx.** Laragon's auto-vhost layer (`beauclick.test`) was the original plan, but the actual verified setup skips it entirely — `wordpress/router.php` (committed) exists precisely so `php -S localhost:8080 router.php`, run from `wordpress/`, can serve pretty permalinks correctly (PHP's built-in server has no `.htaccess`/`mod_rewrite` support; the router forwards any request that isn't a real file to `index.php`, exactly what `mod_rewrite` would otherwise do). This matches `.env`'s `WP_HOME=http://localhost:8080` and needs nothing from Laragon except its MySQL service running — no virtual host to configure or keep in sync. Real hosting (Apache/Nginx in production) doesn't use this router at all; it's dev-server-only. See [README.md](../../README.md#local-development-setup-windows) for the exact commands.

Setup outline: install Laragon, start MySQL from its control panel (Apache/Nginx not needed) → `composer create-project johnpbloch/wordpress` into `wordpress/` → `composer install` in each plugin → `npm install && npm run build` in `app/` → `php -S localhost:8080 router.php` from `wordpress/` → run the WP installer → seed reference data via `wp bc:seed`.
...
- **README.md:** setup steps (Laragon for PHP/MySQL → clone → composer/npm install → `php -S localhost:8080 router.php` → seed DB), plus a link to this architecture doc and the design handoff as the two sources of truth.
...
### Third pass: live verification against the real running site

Once local dev serving was fixed (see the environment note below), the remaining named critical flows were live-verified against the actual WordPress site rather than static analysis: Professional dashboard (Overview/Bookings/Reviews/Messages tabs), Chat end-to-end (customer → professional, unread badge, both directions), and B2B (catalog → add to wholesale order → cart → totals). Two real, confirmed findings, both fixed:

- **B2B wholesale catalog showed the wrong price next to its own "add to order" button (real bug, not a false alarm).** Each product card displayed the *best* (100+ units) tier price as the headline, but the card's only action button added the product at its MOQ — for every seeded product, MOQ fell in the "1–9 units → base price, no discount" tier. A buyer would see e.g. "۳۸۵٬۰۰۰ تومان" and click "افزودن به سفارش عمده", but the cart would actually charge "۵۵۰٬۰۰۰ تومان" — a ~30% gap between the advertised and charged price for the only click available on that card. WooCommerce's actual cart/checkout total was always correct (`TierPricingEngine::apply_tier_prices()` recomputes the real price server-side from actual quantity on every cart calculation — this was never a financial exploit, purely a misleading display), but a real buyer would have felt bait-and-switched. Fixed in `page-b2b.php`: the headline price now comes from `price_for_quantity($product_id, $moq)` — what the button's click actually charges — with the old strikethrough only shown when that price is genuinely a discount, and a new honest secondary line ("در سفارش‌های ۱۰۰+ عدد: …") surfacing the bulk-tier price as an incentive instead of a misrepresentation. Verified live: added the same product to cart before and after the fix — cart total now matches the displayed price exactly.
- WordPress core's own emoji-detection script (`wp-emoji-release.min.js`) pings `s.w.org` for a fallback image on every page load containing an emoji character — dead weight on every modern browser, and an external CDN dependency this Iran-hosted platform has no reason to carry. Disabled via the standard WP recipe in the new `inc/performance.php`.

- **Critical — the entire Admin flow (B2B approval, review moderation) was completely inaccessible, and it was pure luck which one.** Live-verifying the Admin flow (Login → B2B approval → Review moderation → Professional verification) found `/wp-admin/admin.php?page=beauclick-b2b-accounts` outright refusing a real administrator with the correct capability — a generic "you are not allowed to access this page" `wp_die()`. Root cause, traced through WordPress core's own `wp-admin/includes/menu.php` and `user_can_access_admin_page()`: when a top-level menu page (`beauclick-core`'s `AdminMenu`, parent slug `'beauclick'`) registers no submenu matching its OWN slug, WordPress's menu-builder silently "promotes" whichever OTHER submenu happens to register first under that parent into the parent's effective landing page — rewriting the parent slug used everywhere menu links are built. The promoted page's actual permission hook, though, was registered against the ORIGINAL parent at `add_submenu_page()` time, so the runtime lookup computes a hookname that was never registered and denies access outright, regardless of capability. Since `beauclick-b2b`'s and `beauclick-reviews`'s admin pages both register at WordPress's default hook priority, this bug's victim depended entirely on plugin activation order — it could just as easily have been review moderation that broke instead of B2B approval. Fixed in `AdminMenu`: explicitly registers a submenu matching the parent's own slug (the fix WordPress's own developer documentation names for exactly this situation) and hooks at priority 5 so it always registers before any other module's default-priority submenu, regardless of activation order. Two regression tests assert the invariant directly (self-referencing submenu present; hook priority ahead of the default) — verified via `git stash` that both genuinely fail against the pre-fix code, one with the exact "page doesn't exist yet" state that produced the live 403.
- **`wordpress/router.php` (the dev-only PHP built-in server router — see §23) only checked `is_file()`, never `is_dir()`.** A request for `/wp-admin/` itself — not a query-string page, the directory — isn't a file, so it silently fell through to the site's own front-end `index.php` instead of letting PHP's built-in server resolve `wp-admin/index.php` normally, bouncing every attempt to reach wp-admin back to the homepage with no error. This is exactly why Admin-flow verification couldn't even start until it was found and fixed; real hosting (Apache/Nginx) was never affected, since neither uses this file.

- **Mobile QA caught the accessibility pass's own Modal/Badge/Chip CSS fixes never reaching real WordPress pages.** `app/src/design-system/primitives/{Modal,Badge,Chip}.css` were edited earlier this session (focus-trap close button, contrast fixes), but the theme keeps its own hand-synced copy at `wordpress/wp-content/themes/beauclick/assets/css/components.css` for server-rendered pages (see the "design-system ↔ theme parity" FUTURE ENHANCEMENT entry above, about a CI check that doesn't exist yet) — those edits were never mirrored there. On the real site the booking modal's close button rendered completely unstyled (`position: static`, browser-default size) instead of the intended 32×32 corner button, and the badge/chip contrast fixes never took effect at all. Fixed by applying the identical CSS to `components.css`; this is exactly the kind of drift the still-open FUTURE ENHANCEMENT CI check exists to catch automatically instead of relying on live verification to notice it.

Two more apparent bugs surfaced during this pass turned out to be tooling artifacts, not product defects, after being run down to their actual cause rather than reported at face value: (1) a stray "sara ahmadi" `<h1>` greeting appeared to be missing its emoji in DOM text extraction — the built JS bundle genuinely contains it; the extraction tool's handling of the astral (surrogate-pair) character was the actual gap. (2) Recurring 403/404 console errors on the live site traced back to a stale accumulated console buffer in one browser tab (from an earlier wrong-URL guess of my own), not a live, reproducible issue — confirmed by loading the same pages in a fresh tab with zero errors. Both are noted here only so a future pass doesn't waste time re-chasing them.
