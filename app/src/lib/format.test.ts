...
describe( 'formatShortDate (Jalali)', () => {
	...
	it( 'never returns a Gregorian month number where a Jalali month name belongs', () => {
		// August (Gregorian month 8) must not leak through as "8" -- it should
		// resolve to a real Jalali month name (Mordad or Shahrivar depending
		// on the exact day).
