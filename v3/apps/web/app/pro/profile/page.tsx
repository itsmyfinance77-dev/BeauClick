...
export default function ProProfilePage() {
  ...
  if (state === 'loading') return <LoadingState label="در حال بارگذاری پروفایل متخصص…" lines={5} />;
  ...
  return (
    <>
          ...
          <div className={styles.columns}>
            <div className={styles.field}>
              ...
              <fieldset className={styles.fieldset}>
                <legend className={styles.legend}>تخصص‌ها</legend>
                {!refLoaded ? (
      ...
      {profile ? (
        <div className={`${styles.panel} ${styles.verification}`}>
          <h2 className={styles.verifyTitle}>احراز هویت</h2>
          ...
          {profile.verificationStatus === 'verified' ? (
            <p className={styles.verifyText}>پروفایل شما تأیید شده است.</p>
          ) : profile.verificationStatus === 'pending' ? (
            <p className={styles.verifyText}>درخواست شما در صف بررسی است. نتیجه از طریق همین صفحه اعلام می‌شود.</p>
            ...
            <>
              ...
              <Button type="button" inline loading={submitting} onClick={() => void requestVerification()}>
      ...
      {profile ? (
        <p className={styles.footnote}>وضعیت تأیید توسط تیم بررسی پلتفرم تعیین می‌شود و از این صفحه قابل تغییر نیست.</p>
      ) : null}
    </>
  );
}
